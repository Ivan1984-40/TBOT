...
# =================================================================================
GRID_INTERVAL = Decimal('0.2') # Интервал для размещения ордеров в рублях 1.0=1руб
# =================================================================================
...
# =================================================================================
    ...
    async def _sync_position_with_api(self, client, account_id: str, current_lots_held_from_api: Decimal, current_avg_price_from_api: Decimal, last_known_market_price: Decimal):
        ...
        # ==========================================================================
        # === НОВАЯ ЛОГИКА: Полная синхронизация только при старте/рестарте ===
        # ==========================================================================
        
        # --- Шаг 3: Полная сверка и восстановление TP ордеров ---
        self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: Начинаем полную синхронизацию TP ордеров.")

        # --- 3a. Получаем текущую позицию из API ---
        api_lots_count = int(current_lots_held_from_api)
        self.logger.debug(f"[{account_state.account_name_for_log}] FULL_SYNC: Позиция по API: {api_lots_count} лотов.")

        # --- 3b. Получаем внутреннюю позицию из bot_state ---
        internal_lots_list = account_state.open_lots
        internal_lots_count = sum(lot['quantity'] for lot in internal_lots_list)
        self.logger.debug(f"[{account_state.account_name_for_log}] FULL_SYNC: Внутренняя позиция из bot_state: {internal_lots_count} лотов. open_lots: {internal_lots_list}")

        # --- 3c. Сценарий 1: Позиции совпадают и TP полные ---
        if api_lots_count == internal_lots_count and internal_lots_count > 0:
            # Проверяем, все ли лоты имеют активные TP
            all_lots_have_active_tp = True
            for lot in internal_lots_list:
                tp_id = lot.get('tp_order_id')
                if not tp_id or not any(so.stop_order_id == tp_id for so in active_exchange_stop_orders):
                    all_lots_have_active_tp = False
                    break
            
            if all_lots_have_active_tp:
                self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: Позиции совпадают ({api_lots_count}) и все TP активны. Ничего не делаем. Синхронизация завершена.")
                self.logger.info(f"[{account_state.account_name_for_log}] Синхронизация завершена. Текущая внутренняя позиция: {internal_lots_count} лот(ов). account_started: {account_state.account_started}, initial_orders_placed: {account_state.initial_orders_placed}")
                self.save_state()
                return # <-- ВАЖНО: Выходим из метода
            else:
                self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: Позиции совпадают ({api_lots_count}), но не все TP активны. Переходим к восстановлению TP.")
        
        # --- 3d. Сценарий 2: Восстановление старых TP ---
        # Если позиция по API равна или больше внутренней, и во внутреннем состоянии есть лоты с TP
        if api_lots_count >= internal_lots_count and internal_lots_count > 0:
            self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: Позиция API ({api_lots_count}) >= внутренней ({internal_lots_count}) и есть лоты в bot_state. Восстанавливаем TP для существующих лотов.")
            ...
            for lot in internal_lots_list:
                ...
                if tp_id and any(so.stop_order_id == tp_id for so in active_exchange_stop_orders):
                    self.logger.debug(f"[{account_state.account_name_for_log}] FULL_SYNC: TP ордер {tp_id} для лота {entry_price:.{self.display_precision}f} уже активен на бирже. Пропускаем.")
                    continue # TP активен, ничего не делаем
                
                # Если TP не активен, восстанавливаем его
                self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: Восстанавливаем TP ордер для лота {entry_price:.{self.display_precision}f}, количество {quantity} ед.")
                task = self._place_individual_take_profit_order(
                    client, account_id, entry_price, quantity # <-- Используем entry_price лота
                )
                recovery_tasks.append((task, lot)) # Сохраняем лот для обновления tp_order_id
            ...
            if recovery_tasks:
                ...
                for i, (result, lot) in enumerate(zip(task_results, (lot for _, lot in recovery_tasks))):
                    if not isinstance(result, Exception) and result:
                        lot['tp_order_id'] = result # Обновляем tp_order_id в internal_lots_list
                        self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: Восстановлен TP ордер {result} на {lot['quantity']} ед. по цене {lot['entry_price']:.{self.display_precision}f}.")
                    else:
                        self.logger.error(f"[{account_state.account_name_for_log}] FULL_SYNC: НЕ УДАЛОСЬ восстановить TP для лота {lot['entry_price']:.{self.display_precision}f}, количество {lot['quantity']} ед. Ошибка: {result}")
        
        # --- 3e. Шаг добавления недостающих лотов ---
        if api_lots_count > internal_lots_count:
            new_units_to_add_count = api_lots_count - internal_lots_count
            self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: Обнаружено {new_units_to_add_count} новых единиц(ы) по API. Добавляем во внутреннее состояние.")
            
            # Добавляем один лот с количеством равным недостающим единицам
            new_lot = {'entry_price': last_known_market_price, 'tp_order_id': None, 'quantity': new_units_to_add_count}
            internal_lots_list.append(new_lot)
            ...
            self.logger.debug(f"[{account_state.account_name_for_log}] FULL_SYNC: last_position_fill_price обновлена до рыночной цены ({account_state.last_position_fill_price:.{self.display_precision}f}) из-за новых лотов, обнаруженных синхронизацией.")
        
        # --- 3f. Шаг удаления лишних лотов ---
        elif api_lots_count < internal_lots_count:
            units_to_remove_count = internal_lots_count - api_lots_count
            self.logger.warning(f"[{account_state.account_name_for_log}] FULL_SYNC: Обнаружено {units_to_remove_count} единиц(ы), закрытых извне. Уменьшаем количество в лотах.")
            
            # Уменьшаем количество единиц, а не удаляем целые лоты
            remaining_units_to_remove = units_to_remove_count
            new_internal_lots_list = []
            
            for lot in internal_lots_list: # <-- Итерируемся по текущему списку
                if remaining_units_to_remove <= 0:
                    # Если уже убрали всё, что нужно, добавляем лот как есть
                    new_internal_lots_list.append(lot)
                else:
                    lot_quantity = lot['quantity']
                    # --- НОВОЕ: Логика отмены TP перед удалением/уменьшением ---
                    tp_id_to_cancel = lot.get('tp_order_id')
                    if tp_id_to_cancel:
                        self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: Пытаемся отменить TP ордер {tp_id_to_cancel}, связанный с удаляемым/уменьшаемым лотом (количество {lot_quantity} -> {lot_quantity - min(lot_quantity, remaining_units_to_remove)}).")
                        ...
                        try:
                            asyncio.ensure_future(self.cancel_order(client, tp_id_to_cancel, is_stop_order=True, account_id=account_state.account_id))
                             ...
                             self.logger.warning(f"[{account_state.account_name_for_log}] FULL_SYNC: Исключение при попытке асинхронной отмены TP {tp_id_to_cancel}: {e}")
                    ...
                    if lot_quantity <= remaining_units_to_remove:
                        ...
                        self.logger.debug(f"[{account_state.account_name_for_log}] FULL_SYNC: Удален лот на {lot_quantity} единиц.")
                        # TP уже отменен выше
                    else:
                        ...
                        new_internal_lots_list.append(lot)
                        self.logger.debug(f"[{account_state.account_name_for_log}] FULL_SYNC: Уменьшено количество в лоте с {lot_quantity} до {lot['quantity']} единиц.")
                        # TP уже отменен выше. SYNC_TP позже решит, нужно ли восстанавливать/пересоздавать TP для нового количества.
            
            internal_lots_list = new_internal_lots_list # <-- Обновляем список лотов    
            ...
            self.logger.debug(f"[{account_state.account_name_for_log}] FULL_SYNC: last_position_fill_price обновлена до рыночной цены ({account_state.last_position_fill_price:.{self.display_precision}f}) из-за единиц, закрытых извне.")
        
        # --- 3g. Шаг добавления недостающих TP для новых/обновленных лотов ---
        # Подсчитываем общее количество единиц без TP
        units_without_tp_total = sum(lot['quantity'] for lot in internal_lots_list if not lot.get('tp_order_id'))
        
        if units_without_tp_total > 0:
            self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: Нужно разместить TP ордер(а) на {units_without_tp_total} единиц.")
            
            # Используем цену первого лота без TP для расчета TP
            # Для новых единиц (добавленных в шаге 3e) лучше использовать last_known_market_price
            # Но если internal_lots_list это существующие лоты без TP, используем их entry_price
            # Простое решение: используем entry_price первого лота из internal_lots_list без TP
            base_price_for_new_tp = next((lot['entry_price'] for lot in internal_lots_list if not lot.get('tp_order_id')), last_known_market_price)
    
            # Размещаем один TP ордер на все недостающие единицы
            new_tp_order_id = await self._place_individual_take_profit_order(
                client, account_id, base_price_for_new_tp, units_without_tp_total
            )
    
            if new_tp_order_id:
                # Привязываем этот TP ордер ко всем лотам без TP
                for lot in internal_lots_list:
                    if not lot.get('tp_order_id'):
                        lot['tp_order_id'] = new_tp_order_id
                self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: TP ордер {new_tp_order_id} размещен успешно на {units_without_tp_total} единиц по цене {base_price_for_new_tp:.{self.display_precision}f}.")
            else:
                self.logger.error(f"[{account_state.account_name_for_log}] FULL_SYNC: НЕ УДАЛОСЬ РАЗМЕСТИТЬ TP ордер на {units_without_tp_total} единиц.")
        elif units_without_tp_total == 0:
             self.logger.info(f"[{account_state.account_name_for_log}] FULL_SYNC: Все лоты уже имеют TP ордера или лотов нет.")
        
        # Обновляем основной список лотов
        account_state.open_lots = internal_lots_list
        self.logger.debug(f"[{account_state.account_name_for_log}] FULL_SYNC: Обновленный open_lots: {account_state.open_lots}")

        # --- Шаг 4: Обновление флагов состояния бота на основе синхронизированной позиции ---
