...
# =================================================================================
INTERVAL_CHECK = 20 # Интервал для периодической проверки позиции и логирования
# =================================================================================
...
#########часть 4\6  ###########
    ...
    async def _sync_position_with_api(self, client, account_id: str, current_lots_held_from_api: Decimal, current_avg_price_from_api: Decimal, last_known_market_price: Decimal):
        ...
        self.logger.debug(f"[{account_state.account_name_for_log}] SYNC: Обновленный open_lots: {account_state.open_lots}")
         ...
         # --- Шаг 4: Размещение недостающих TP ордеров (Упрощенная и надежная логика) ---
        self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: Начинаем размещение TP ордеров.")
        tp_placement_tasks = []
        lots_for_tp = [] # Список лотов, для которых нужно разместить TP

        # Определяем, для каких лотов нужно разместить TP
        for lot in account_state.open_lots:
            if lot.get('tp_order_id') is None:
                lots_for_tp.append(lot)
                self.logger.debug(f"[{account_state.account_name_for_log}] SYNC_TP: Нужно разместить TP для лота {lot['entry_price']:.{self.display_precision}f}.")

        if lots_for_tp:
            self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: Размещаем TP ордера для {len(lots_for_tp)} лотов.")
            # Создаем задачи на размещение
            for lot in lots_for_tp:
            ...
            if tp_placement_tasks:
                ...
                for i, (lot, result) in enumerate(zip(lots_for_tp, tp_results)):
                    if not isinstance(result, Exception) and result:
                        lot['tp_order_id'] = result
                        # Обновляем ID в основном списке account_state.open_lots
                        for original_lot in account_state.open_lots:
                             if original_lot is lot: # Сравнение по ссылке
                                 original_lot['tp_order_id'] = result
                                 self.logger.debug(f"[{account_state.account_name_for_log}] SYNC_TP: Обновлен tp_order_id для лота {lot['entry_price']:.{self.display_precision}f} в основном списке.")
                                 break
                        self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: TP ордер размещен успешно для лота с ценой входа {lot['entry_price']:.{self.display_precision}f}.")
                    else:
                        self.logger.error(f"[{account_state.account_name_for_log}] SYNC_TP: НЕ УДАЛОСЬ РАЗМЕСТИТЬ TP для лота с ценой входа {lot['entry_price']:.{self.display_precision}f}. Ошибка: {result}. Лот останется без TP!")
        else:
             self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: Все лоты уже имеют TP ордера или лотов нет.")
        # --- Конец Шага 4 ---
