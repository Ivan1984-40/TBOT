    ...
    async def manage_account_orders(self, client, account_id: str, current_market_price: Decimal):
        ...
        async with account_state._state_lock:
            ...
            if should_reposition_orders:
                if base_price_for_this_cycle and base_price_for_this_cycle != Decimal('0'):
                    # === ВАЖНО: Передаем ЗАРАНЕЕ определенную ФИКСИРОВАННУЮ цену ===
                    await self._place_directional_entry_orders(client, account_id, base_price_for_this_cycle)
                    # Сброс флага
                    account_state.orders_need_reposition = False 
                    # Обновление времени последнего размещения
                    account_state.last_orders_placement_time = datetime.now() 
                    self.logger.debug(f"[{account_state.account_name_for_log}] Флаг 'orders_need_reposition' сброшен. Время последнего размещения обновлено.")
                else:
                    self.logger.warning(f"[{account_state.account_name_for_log}] Откладываем перестановку ордеров для входа, так как не удалось определить подходящую базовую цену (base_price_for_this_cycle = {base_price_for_this_cycle}).")
            else:
