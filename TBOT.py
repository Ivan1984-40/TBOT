...
# =================================================================================
GRID_INTERVAL = Decimal('0.25') # Интервал для размещения ордеров в рублях 1.0=1руб
# =================================================================================
...
MAX_FREE_MARGIN_RUB_FOR_SELL = Decimal('1100') # Максимальная сумма используемой маржи для SELL счета, чтобы избежать комиссий
# =================================================================================
# =================================================================================
# =================================================================================
...
#########часть 2\6  ###########
    ...
    async def _place_directional_entry_orders(self, client, account_id: str, base_price: Decimal):
        ...
        if ENABLE_TREND_FILTERING and not account_state.is_active_by_trend:
            self.logger.info(f"[{account_state.account_name_for_log}] Счет неактивен по тренду. Пропускаем размещение ордеров для входа.")
            return
        ...
        async with self.orders_placement_lock:
            ...
            self.logger.info(f"[{account_state.account_name_for_log}] Размещение/обновление ордеров для входа. Базовая цена: {base_price:.{self.display_precision}f}")
            ...
            if account_state.is_sell_account: # SELL Account (Shorting)
                ...
                if upper_price_quantized > 0:
                    ...
                    self.logger.info(f"[{account_state.account_name_for_log}] Проверка маржи для ордера ВЫШЕ (SELL LIMIT). Цена: {upper_price:.{self.display_precision}f}. Доступно: {available_margin:.{self.display_precision}f} RUB. Требуется: {required_margin_upper_limit:.{self.display_precision}f} RUB.")
                    ...
                    if available_margin >= required_margin_upper_limit:
                        ...
                        self.logger.warning(f"[{account_state.account_name_for_log}] Лимит использования маржи (потенциальное обеспечение) достигнут ({margin_limit:.{self.display_precision}f} RUB). Ордер ВЫШЕ (SELL LIMIT) не размещается.")
                else:
                ...
                if lower_price_quantized > 0:
                    ...
                    self.logger.info(f"[{account_state.account_name_for_log}] Проверка маржи для ордера НИЖЕ (SELL STOP-LOSS). Цена: {lower_price:.{self.display_precision}f}. Доступно: {available_margin:.{self.display_precision}f} RUB. Требуется: {required_margin_lower_stop:.{self.display_precision}f} RUB.")
                    ...
                    if available_margin >= required_margin_lower_stop:
                        ...
                        self.logger.warning(f"[{account_state.account_name_for_log}] Лимит использования маржи (потенциальное обеспечение) достигнут ({margin_limit:.{self.display_precision}f} RUB). Ордер НИЖЕ (SELL STOP-LOSS) не размещается.")
                else:
                ...
                if lower_price_quantized > 0:
                    ...
                    self.logger.info(f"[{account_state.account_name_for_log}] Проверка маржи для ордера НИЖЕ (BUY LIMIT). Цена: {lower_price:.{self.display_precision}f}. Доступно: {available_margin:.{self.display_precision}f} RUB. Требуется: {required_margin_lower_limit:.{self.display_precision}f} RUB.")
                    ...
                    if available_margin >= required_margin_lower_limit:
                        ...
                        self.logger.warning(f"[{account_state.account_name_for_log}] Лимит использования маржи (потенциальное обеспечение) достигнут ({margin_limit:.{self.display_precision}f} RUB). Ордер НИЖЕ (BUY LIMIT) не размещается.")
                else:
                ...
                if upper_price_quantized > 0:
                    ...
                    self.logger.info(f"[{account_state.account_name_for_log}] Проверка маржи для ордера ВЫШЕ (BUY STOP-LOSS). Цена: {upper_price:.{self.display_precision}f}. Доступно: {available_margin:.{self.display_precision}f} RUB. Требуется: {required_margin_upper_stop:.{self.display_precision}f} RUB.")
                    ...
                    if available_margin >= required_margin_upper_stop:
                        ...
                        self.logger.warning(f"[{account_state.account_name_for_log}] Лимит использования маржи (потенциальное обеспечение) достигнут ({margin_limit:.{self.display_precision}f} RUB). Ордер ВЫШЕ (BUY STOP-LOSS) не размещается.")
                else:
