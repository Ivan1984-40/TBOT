...
#########часть 4\6  ###########
    ...
    async def _sync_position_with_api(self, client, account_id: str, current_lots_held_from_api: Decimal, current_avg_price_from_api: Decimal, last_known_market_price: Decimal):
        ...
        self.logger.debug(f"[{account_state.account_name_for_log}] SYNC: Обновленный open_lots: {account_state.open_lots}")
                ...
                # --- Шаг 4: Размещение недостающих TP ордеров (Упрощенная логика, как в старой версии) ---
        lots_without_tp = len([l for l in account_state.open_lots if l['tp_order_id'] is None])
        self.logger.info(f"[{account_state.account_name_for_log}] SYNC: Размещаем недостающие TP ордера для {lots_without_tp} лотов.")

        tp_placement_tasks = []
        lots_awaiting_tp_id = [] # Для сопоставления результатов с конкретными лотами

        for lot in account_state.open_lots:
            if lot['tp_order_id'] is None: # Простая проверка, как раньше
        ...
        if tp_placement_tasks:
            ...
            for i, result in enumerate(tp_results):
                ...
                if not isinstance(result, Exception) and result:
                    ...
                    # Обновляем ID в основном списке account_state.open_lots, если он был скопирован
                    # во время reconciled_open_lots. Это критично.
                    # Более надежный способ поиска оригинального лота:
                    for original_lot in account_state.open_lots:
                         # Сравниваем по ссылке или по уникальным характеристикам
                         if (original_lot is lot) or \
                            (original_lot.get('entry_price') == lot.get('entry_price') and \
                             original_lot.get('quantity') == lot.get('quantity') and \
                             original_lot.get('tp_order_id') is None):
                             original_lot['tp_order_id'] = result
                             self.logger.debug(f"[{account_state.account_name_for_log}] SYNC: Обновлен tp_order_id для лота {lot['entry_price']:.{self.display_precision}f} в основном списке.")
                             break
                    self.logger.info(f"[{account_state.account_name_for_log}] SYNC: TP ордер размещен успешно для лота с ценой входа {lot['entry_price']:.{self.display_precision}f}.")
                else:
            ...
            if lots_without_tp > 0:
                 self.logger.info(f"[{account_state.account_name_for_log}] SYNC: Все {lots_without_tp} лотов уже имеют TP ордера.")
            else:
