...
# =================================================================================
GRID_INTERVAL = Decimal('0.5') # Интервал для размещения ордеров в рублях 1.0=1руб
# =================================================================================
...
# =================================================================================
LOG_LEVEL_SETTING = "DEBUG" # Can be "DEBUG" or "INFO"
# =================================================================================
...
TREND_CHECK_INTERVAL_SECONDS =7200 # Интервал (в секундах) для пересчета тренда (например, 300с = 5 минут)
TREND_THRESHOLD_PERCENT = Decimal('1.0') # Процент изменения цены для определения тренда (0.05% = 0.0005)
# ===========================================================================================
...
#########часть 2\6  ###########
    ...
    #    str или None: ID размещённого ордера или None в случае ошибки.
        ...
        try:
            ...
            self.logger.info(f"{account_state.account_name_for_log}-размещен ордер TP, цена {tp_price_quantized:.{self.display_precision}f}, количество лотов {quantity}")
            # =========================================================
            return response_id
        except Exception as e:
    ...
    async def _place_directional_entry_orders(self, client, account_id: str, tp_price_quantized: Decimal):
        ...
        async with self.orders_placement_lock:
            ...
            self.logger.debug(f"[{account_state.account_name_for_log}] Размещение/обновление ордеров для входа. Базовая цена: {tp_price_quantized:.{self.display_precision}f}")
            ...
            if account_state.is_sell_account: # SELL Account (Shorting)
                ...
                upper_price = tp_price_quantized + account_state.grid_interval 
                ...
                lower_price = tp_price_quantized - account_state.grid_interval 
...
#########часть 4\6  ###########
    ...
    #Использует информацию об исполненных/отмененных стоп-ордеров.
    #Внедрена проверка существующих активных TP ордеров перед размещением новых.
        ...
        try:
         ...
         # === НОВОЕ: Фильтрация активных TP ордеров ===
        # Отфильтруем только активные TP ордера для данного инструмента
        # ИСПРАВЛЕНИЕ: Используем правильный атрибут 'order_type' из схемы StopOrder
        active_take_profit_orders_on_exchange = [
            order for order in active_exchange_stop_orders
            # ИСПРАВЛЕНО: order.stop_order_type -> order.order_type
            if order.order_type == StopOrderType.STOP_ORDER_TYPE_TAKE_PROFIT
        ]
        self.logger.debug(f"[{account_state.account_name_for_log}] SYNC_TP: Получено {len(active_take_profit_orders_on_exchange)} активных TP ордеров с биржи для синхронизации.")
        # ===========================================
        ...
        # === НОВОЕ: Анализ существующих TP ордеров перед размещением новых ===
        # Подсчитаем общее количество лотов, "покрываемых" активными TP ордерами на бирже
        # lots_requested у StopOrder - это просто int, а не Quotation
        lots_covered_by_active_tp_on_exchange = Decimal('0')
        for tp_order in active_take_profit_orders_on_exchange:
            # lots_requested - это просто целое число
            lots_for_this_tp = Decimal('0')
            if hasattr(tp_order, 'lots_requested') and tp_order.lots_requested is not None:
                 # Преобразуем int в Decimal напрямую
                 lots_for_this_tp = Decimal(str(tp_order.lots_requested))
            # quantity также может присутствовать, проверим на всякий случай
            elif hasattr(tp_order, 'quantity') and tp_order.quantity is not None:
                 if isinstance(tp_order.quantity, (int, float)):
                     lots_for_this_tp = Decimal(str(tp_order.quantity))
                 else:
                     # Если quantity - это Quotation, используем _quotation_to_decimal
                     lots_for_this_tp = _quotation_to_decimal(tp_order.quantity)
        
            lots_covered_by_active_tp_on_exchange += lots_for_this_tp

        self.logger.debug(f"[{account_state.account_name_for_log}] SYNC_TP: Лотов, покрываемых активными TP на бирже: {lots_covered_by_active_tp_on_exchange}")
        # ===================================================================

        # === НОВОЕ: Расчет реально нужных TP и размещение ===
        # Рассчитываем, сколько лотов на самом деле нуждаются в TP ордерах
        # Это разница между тем, сколько TP нам нужно (внутренние лоты) 
        # и тем, сколько TP уже есть на бирже.
        lots_actually_needing_tp = max(Decimal('0'), len([l for l in account_state.open_lots if l['tp_order_id'] is None]) - lots_covered_by_active_tp_on_exchange)
    
        self.logger.info(
            f"[{account_state.account_name_for_log}] SYNC_TP: "
            f"Внутренних лотов без TP: {len([l for l in account_state.open_lots if l['tp_order_id'] is None])}. "
            f"Лотов, покрываемых активными TP на бирже: {lots_covered_by_active_tp_on_exchange}. "
            f"Лотов, реально нуждающихся в новых TP: {lots_actually_needing_tp}."
        )

        # --- Шаг 4: Размещение недостающих TP ордеров ---
        # if len([l for l in account_state.open_lots if l['tp_order_id'] is None]) > 0: # Старая проверка
        if lots_actually_needing_tp > 0: # Новая проверка
            # self.logger.info(f"[{account_state.account_name_for_log}] SYNC: Размещаем недостающие TP ордера для {len([l for l in account_state.open_lots if l['tp_order_id'] is None])} лотов.") # Старое сообщение
            self.logger.info(f"[{account_state.account_name_for_log}] SYNC: Размещаем недостающие TP ордера для {lots_actually_needing_tp} лотов.") # Новое сообщение
            tp_placement_tasks = []
            lots_awaiting_tp_id = [] # Для сопоставления результатов с конкретными лотами

            # Проходим по лотам без TP и размещаем ордера, но только для тех, кому они действительно нужны
            lots_count_needing_tp_placed = Decimal('0')
            for lot in account_state.open_lots:
                if lot['tp_order_id'] is None and lots_count_needing_tp_placed < lots_actually_needing_tp:
                    ...
                    lots_count_needing_tp_placed += Decimal(lot['quantity']) # Увеличиваем счетчик размещаемых TP
            ...
            # Если lots_actually_needing_tp < количества лотов без TP, остальные лоты без TP остаются как есть.
            # Это означает, что для них TP уже существуют на бирже.
        else:
            # Упрощаем сообщение, если TP не нужны
            if len(account_state.open_lots) > 0:
                self.logger.info(f"[{account_state.account_name_for_log}] SYNC: Активных TP достаточно для всех {len(account_state.open_lots)} внутренних лотов. Дополнительные TP не размещаются.")
            else:
                 self.logger.info(f"[{account_state.account_name_for_log}] SYNC: Нет внутренних лотов. TP ордера не требуются.")
        # ===================================================
...
#########часть 5\6  ###########
    async def manage_account_orders(self, client, account_id: str, current_market_price: Decimal):
        ...
        async with account_state._state_lock:
            ...
            # === УПРОЩЕННАЯ ЛОГИКА: Фиксация базовой цены для этой итерации ===
            # ВСЕГДА используем текущую рыночную цену для расчета ВХОДНЫХ ордеров (выше/ниже) и TP.
            # Это обеспечивает согласованность сетки, независимо от того, синхронизация это, обычная итерация
            # или реакция на исполнение.
            base_price_for_this_cycle = None
            base_price_source = "unknown"

            if current_market_price and current_market_price != Decimal('0'):
                base_price_for_this_cycle = current_market_price
                base_price_source = "current_market_price"
                self.logger.debug(f"[{account_state.account_name_for_log}] Используем current_market_price как base_price для входных ордеров и TP: {base_price_for_this_cycle} (источник: {base_price_source})")
            else:
                # Запасной вариант - средняя цена из API
                # ПРЕДПОЛОЖЕНИЕ: current_avg_price_from_api доступна в этой области видимости
                # или её нужно получить здесь же, как это делается ниже для portfolio.
                # Для простоты, оставим логику как есть, но учтем, что current_avg_price_from_api
                # должна быть определена до этого момента или внутри блокировки.
                # В реальном коде, её лучше получить из portfolio внутри блокировки.
                # current_avg_price_from_api = ... (получение из portfolio, см. ниже)
            
                # Для демонстрации, предположим, что мы получаем её внутри блокировки
                portfolio_for_avg = None
                try:
                    # Получаем portfolio внутри блокировки, чтобы состояние было согласованным
                    portfolio_for_avg = await client.operations.get_portfolio(account_id=account_id)
                except Exception as e:
                    self.logger.warning(f"[{account_state.account_name_for_log}] Ошибка получения портфолио для расчета средней цены внутри блокировки: {e}")
            
                current_avg_price_from_api_inside_lock = Decimal('0')
                if portfolio_for_avg:
                     current_figi_position_for_avg = next((pos for pos in portfolio_for_avg.positions if pos.figi == self.figi), None)
                     if current_figi_position_for_avg and current_figi_position_for_avg.average_position_price:
                          current_avg_price_from_api_inside_lock = _quotation_to_decimal(current_figi_position_for_avg.average_position_price)

                if current_avg_price_from_api_inside_lock and current_avg_price_from_api_inside_lock != Decimal('0'):
                    base_price_for_this_cycle = current_avg_price_from_api_inside_lock
                    base_price_source = "average_position_price (fallback)"
                    self.logger.debug(f"[{account_state.account_name_for_log}] Используем average_position_price как base_price для входных ордеров и TP (fallback): {base_price_for_this_cycle} (источник: {base_price_source})")
                else:
                    # Если совсем нет цены, используем 0, вызывающий код должен обработать это
                    base_price_for_this_cycle = Decimal('0')
                    base_price_source = "unavailable"
                    self.logger.warning(f"[{account_state.account_name_for_log}] Не удалось определить рыночную цену для входных ордеров и TP. Установлена 0.")

            # Логируем зафиксированную цену для всей итерации
            self.logger.info(f"[{account_state.account_name_for_log}] Цена для расчета сетки и ТР зафиксирована: {base_price_for_this_cycle} (источник: {base_price_source})")
            # ================================================

            await self._reset_daily_flags_if_new_day(client, account_id, current_market_price) # Предполагается, что этот метод потокобезопасен или не изменяет account_state
            if account_state.daily_loss_limit_hit:
                ...
                # Возврат из блокировки происходит автоматически
                return 
        
            # Предполагается, что _check_margin_and_manage_orders не изменяет account_state напрямую,
            # или вызывает потокобезопасные методы. Если это не так, его тоже нужно обернуть.
            # await self._check_margin_and_manage_orders(client, account_id) 

            # Always fetch portfolio first to get the freshest API state
            # Переносим получение portfolio внутрь блокировки для согласованности
            portfolio = None
            try:
                portfolio = await client.operations.get_portfolio(account_id=account_id)
            except RequestError as e:
                self.logger.warning(f"Ошибка получения портфолио для {account_state.account_name_for_log} внутри блокировки: {e}. Пропускаем текущую проверку.")
                # Возврат из блокировки происходит автоматически
                return
            except Exception as e:
                self.logger.error(f"Непредвиденная ошибка при получении портфолио для {account_state.account_name_for_log} внутри блокировки: {e}", exc_info=True)
                # Возврат из блокировки происходит автоматически
                return
            ...
            self.logger.debug(f"[{account_state.account_name_for_log}] (Внутри блокировки) Позиция по портфолио (API): {int(current_lots_held_from_api)} лот(ов) по средней цене {current_avg_price_from_api:.{self.display_precision}f}.")
            self.logger.debug(f"[{account_state.account_name_for_log}] (Внутри блокировки) Текущая рыночная цена (Бот): {current_market_price:.{self.display_precision}f}.")
            self.logger.debug(f"[{account_state.account_name_for_log}] (Внутри блокировки) Позиция внутренняя (Бот): {internal_lots_count} лот(ов) по средней цене {sum(l['entry_price'] * l['quantity'] for l in account_state.open_lots) / internal_lots_count if internal_lots_count > 0 else Decimal('0'):.{self.display_precision}f}.")
            self.logger.debug(f"[{account_state.account_name_for_log}] (Внутри блокировки) Account started: {account_state.account_started}, Initial orders placed: {account_state.initial_orders_placed}, Orders need reposition: {account_state.orders_need_reposition}")
            self.logger.debug(f"[{account_state.account_name_for_log}] (Внутри блокировки) Internal open_lots_list: {account_state.open_lots}")
            ...
            if needs_sync:
                # ВАЖНО: _sync_position_with_api также должен использовать ту же блокировку account_state._state_lock.
                # Чтобы избежать deadlock (повторный захват той же блокировки), убедись, что
                # _sync_position_with_api НЕ оборачивает свою логику в async with account_state._state_lock.
                # Он будет выполняться в уже захваченной блокировке.
                ...
                # Пересчитываем внутреннюю позицию и API-позицию ПОСЛЕ синхронизации, всё еще внутри блокировки
                portfolio_after_sync = await client.operations.get_portfolio(account_id=account_id)
                current_figi_position_after_sync = next((pos for pos in portfolio_after_sync.positions if pos.figi == self.figi), None)
                current_lots_held_from_api_after_sync = abs(_quotation_to_decimal(current_figi_position_after_sync.quantity_lots)).normalize() if current_figi_position_after_sync else Decimal('0')
                current_avg_price_from_api_after_sync = _quotation_to_decimal(current_figi_position_after_sync.average_position_price) if current_figi_position_after_sync and current_figi_position_after_sync.average_position_price else Decimal('0')
                internal_lots_count_after_sync = sum(l['quantity'] for l in account_state.open_lots)
                self.logger.debug(f"[{account_state.account_name_for_log}] (После синхронизации, внутри блокировки) Внутренняя позиция: {internal_lots_count_after_sync}, API: {int(current_lots_held_from_api_after_sync)}. account_started: {account_state.account_started}, initial_orders_placed: {account_state.initial_orders_placed}")

            # Initial market order placement logic - now after all reconciliation
            self.logger.debug(f"[{account_state.account_name_for_log}] Проверка начального рыночного ордера: PLACE_INITIAL_MARKET_ORDER={PLACE_INITIAL_MARKET_ORDER}, internal_lots_count={internal_lots_count_after_sync if needs_sync else internal_lots_count}, current_lots_held_from_api={int(current_lots_held_from_api_after_sync if needs_sync else current_lots_held_from_api)}, initial_orders_placed={account_state.initial_orders_placed}, account_started={account_state.account_started}")
            # Исправлено: добавлена проверка account_started, чтобы не размещать начальный ордер, если счет уже "начал работу"
            if (PLACE_INITIAL_MARKET_ORDER and 
                (internal_lots_count_after_sync if needs_sync else internal_lots_count) == 0 and 
                int(current_lots_held_from_api_after_sync if needs_sync else current_lots_held_from_api) == 0 and 
                not account_state.initial_orders_placed and
                not account_state.account_started): # <-- Добавлено
                if not account_state.is_sell_account:
                    ...
                    self.save_state() # Предполагается, что save_state потокобезопасен
                    ...
                    if not market_order_id:
                        ...
                        account_state.pending_initial_market_order_id = market_order_id
                    self.save_state() # Предполагается, что save_state потокобезопасен
            # Исправлено: добавлено логирование пропущенного размещения
            elif (PLACE_INITIAL_MARKET_ORDER and 
                  (internal_lots_count_after_sync if needs_sync else internal_lots_count) == 0 and 
                  int(current_lots_held_from_api_after_sync if needs_sync else current_lots_held_from_api) == 0 and 
                  not account_state.initial_orders_placed and
                  account_state.account_started): # <-- Условие, когда пропускаем из-за account_started
                 self.logger.debug(f"[{account_state.account_name_for_log}] Пропущено размещение начального рыночного ордера: счет уже начал торговлю (account_started=True).")
            ...
            # effective_base_price_for_placement = current_market_price # УДАЛЕНО: теперь используем зафиксированную цену
            # base_price_for_this_cycle уже определена выше

            # Проверяем активность ордеров на бирже через OrderManager
            # Доступ к account_state.* происходит внутри блокировки
            upper_id, upper_is_stop = account_state.active_entry_order_above
            lower_id, lower_is_stop = account_state.active_entry_order_below
            ...
            # Упрощаем логику: если флаг установлен, переставляем. Иначе проверяем таймер и активность.
            if account_state.orders_need_reposition:
                ...
                time_since_last_placement = (datetime.now() - account_state.last_orders_placement_time).total_seconds()
                if time_since_last_placement >= account_state.check_grid_interval_seconds:
                    if not has_upper_entry_order_active or not has_lower_entry_order_active:
                        self.logger.info(f"[{account_state.account_name_for_log}] Прошло достаточно времени ({int(time_since_last_placement)}s >= {account_state.check_grid_interval_seconds}s) и один из ордеров сетки неактивен. Инициируем перестановку.")
                        should_reposition_orders = True
                    else:
                    ...
                    self.logger.debug(f"Для {account_state.account_name_for_log}: Периодическая проверка ордеров для входа пропущена (недостаточно времени прошло: {int(time_since_last_placement)}s < {account_state.check_grid_interval_seconds}s).")

                # Если нужно переставить ордера, размещаем новые
          # Добавляем проверку account_started, чтобы не размещать ордера, если счет "не начал работу"
          # Это предотвращает зацикливание при сбросе account_started=False в _sync_position_with_api
            if should_reposition_orders and account_state.account_started: # <-- Добавлено условие
                  if base_price_for_this_cycle != Decimal('0'):
                      await self._place_directional_entry_orders(client, account_id, base_price_for_this_cycle)
                      account_state.orders_need_reposition = False # Сбрасываем флаг после успешного размещения
                  else:
            ...
            elif should_reposition_orders and not account_state.account_started: # <-- Добавлено логирование пропущенного размещения
                   self.logger.debug(f"[{account_state.account_name_for_log}] Пропущено размещение ордеров для входа: счет не начал торговлю (account_started=False), несмотря на установленный флаг orders_need_reposition.")
                    # Сохраняем состояние в конце итерации, всё еще внутри блокировки
                   self.save_state() # Предполагается, что save_state потокобезопасен

                    # === КОНЕЦ защищенной секции ===
                    # Блокировка account_state._state_lock автоматически освобождается здесь.
                    # =============================  
