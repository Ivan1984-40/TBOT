...
from tinkoff.invest._errors import RequestError, AioRequestError
...
# =================================================================================
LOTS = 2 # Количество лотов на каждый ордер (SH, SL, BL, BH)
# =================================================================================
GRID_INTERVAL = Decimal('0.3') # Интервал для размещения ордеров в рублях 1.0=1руб
# =================================================================================
...
# =================================================================================
CHECK_GRID_INTERVAL_SECONDS = 5 # Периодичность проверки и перестановки ордеров для входа, если они устарели
# =================================================================================
...
MAX_FREE_MARGIN_RUB_FOR_BUY = Decimal('500') # Максимальная сумма используемой маржи для BUY счета, чтобы избежать комиссий
MAX_FREE_MARGIN_RUB_FOR_SELL = Decimal('2500') # Максимальная сумма используемой маржи для SELL счета, чтобы избежать комиссий
# =================================================================================
# =================================================================================
# =================================================================================
ACCOUNT_FOR_TRADING = 1 # 1 (BUY), 2 (SELL), 3 (BUY-SELL)
# =================================================================================
...
TREND_THRESHOLD_PERCENT = Decimal('0.75') # Процент изменения цены для определения тренда (0.05% = 0.0005)
# ===========================================================================================
...
class AccountStateData:
    ...
    is_active_by_trend: bool = True # NEW: Флаг активности счета по тренду
     # --- НОВОЕ: Блокировка для синхронизации доступа к состоянию ---
    # Используем default_factory для создания уникальной блокировки для каждого экземпляра
    _state_lock: asyncio.Lock = field(default_factory=asyncio.Lock)
    # ---------------------------------------------------------------
    def to_dict(self):
        # Создаем словарь, исключая _state_lock
        data = {
            "account_id": self.account_id,
            ...
            "is_active_by_trend": self.is_active_by_trend,
            # _state_lock НЕ включается
        }
        return data
...
class OrderManager:
    ...
    def __init__(self, logger: logging.Logger, bot_client_code: str, figi: str, min_price_increment: Decimal, display_precision: int):
        ...
        self._sync_lock = asyncio.Lock()  # Для предотвращения race condition
...
# =================================================================================
    ...
    async def place_limit_order(self, client: AsyncClient, price: Decimal, quantity: int, direction: OrderDirection, order_tag: str, account_id: str) -> Optional[str]:
        ...
        try:
            ...
            # --- Добавлено: Проверка на "Instrument is not available for trading" ---
            if isinstance(e, AioRequestError):
                error_code = str(e.code) if hasattr(e, 'code') else ''
                # Проверяем код ошибки 30079 и сообщение в metadata
                if error_code == '30079' and hasattr(e, 'metadata') and e.metadata and \
                   hasattr(e.metadata, 'message') and 'Instrument is not available for trading' in e.metadata.message:
                    self.logger.warning(f"[OrderManager] Инструмент {self.figi} недоступен для торговли. Не удалось разместить лимитный ордер для счета {account_id}. Причина: {e.metadata.message}")
                    # Можно также установить флаг или выполнить другую логику, если нужно
                    # Например, остановить бота или перейти в режим ожидания
                    # self.parent_bot.trading_halted = True # если есть ссылка на родительский бот
                    return None # Возвращаем None, как и в других ошибках
    ...
    async def place_stop_order(self, client: AsyncClient, stop_price: Decimal, quantity: int, direction: OrderDirection, stop_order_type: StopOrderType, limit_price: Optional[Decimal], order_tag: str, account_id: str) -> Optional[str]:
        ...
        try:
            ...
            if isinstance(e, AioRequestError):
                ...
                if error_code == '30079' and hasattr(e, 'metadata') and e.metadata and \
                   hasattr(e.metadata, 'message') and 'Instrument is not available for trading' in e.metadata.message:
                    self.logger.warning(f"[OrderManager] Инструмент {self.figi} недоступен для торговли. Не удалось разместить стоп-ордер для счета {account_id}. Причина: {e.metadata.message}")
                    return None
            # --- Конец добавления ---
    ...
    async def place_market_order(self, client: AsyncClient, quantity: int, direction: OrderDirection, order_tag: str, account_id: str) -> Optional[str]:
        ...
        try:
            ...
            metadata_message = ""
            if hasattr(e, 'metadata') and e.metadata and hasattr(e.metadata, 'message'):
                metadata_message = e.metadata.message

            if error_code == '30079' and 'Instrument is not available for trading' in metadata_message:
                self.logger.warning(f"[OrderManager] Инструмент {self.figi} недоступен для торговли. Не удалось разместить рыночный ордер для счета {account_id}. Причина: {metadata_message}")
                return None
            # --- Конец добавления ---
            
            if error_code == '30079':
                self.logger.error(f"ОШИБКА: Инструмент {TICKER} недоступен для торговли. Не удалось разместить рыночный ордер для счета {account_id}. {error_message}")
            else:
                self.logger.error(f"[OrderManager] Ошибка при размещении MARKET ордера для {account_id}: {e}", exc_info=True)
            return None
        except Exception as e:
            # --- Добавлено: Проверка на "Instrument is not available for trading" для общего Exception ---
            # На случай, если ошибка придет не как RequestError, но будет AioRequestError
            if isinstance(e, AioRequestError):
                 error_code = str(e.code) if hasattr(e, 'code') else ''
                 if error_code == '30079' and hasattr(e, 'metadata') and e.metadata and \
                    hasattr(e.metadata, 'message') and 'Instrument is not available for trading' in e.metadata.message:
                     self.logger.warning(f"[OrderManager] Инструмент {self.figi} недоступен для торговли. Не удалось разместить рыночный ордер для счета {account_id}. Причина: {e.metadata.message}")
                     return None
            # --- Конец добавления ---
    ...
    async def _place_individual_take_profit_order(self, client, account_id: str, base_price_for_tp: Decimal, quantity: int):
    
    #Размещает индивидуальный тейк-профит ордер для одного лота.
    
    #Args:
    #    client: Асинхронный клиент Tinkoff Invest.
    #    account_id: ID счета.
    #    base_price_for_tp: Базовая цена для расчета TP (обычно зафиксированная рыночная цена итерации).
    #    quantity: Количество лотов для закрытия.
        
    #Returns:
    #    str или None: ID размещённого ордера или None в случае ошибки.
        ...
        # === ИЗМЕНЕНО: Расчет TP на основе base_price_for_tp ===
        # ВСЕГДА используем переданную base_price_for_tp и интервал сетки.
        # Это гарантирует согласованность TP с входными ордерами, размещенными на основе той же цены.
        # Для шорта (счет SELL): TP ниже базовой цены (base_price_for_tp - interval)
        # Для лонга (счет BUY): TP выше базовой цены (base_price_for_tp + interval)
        raw_tp_price = base_price_for_tp - account_state.grid_interval if account_state.is_sell_account else base_price_for_tp + account_state.grid_interval
        # ===================================================================
    
        # Квантуем цену TP
        tp_price_quantized = (raw_tp_price / self.min_price_increment).quantize(Decimal('1.'), rounding=ROUND_HALF_UP) * self.min_price_increment
    
        if tp_price_quantized <= 0:
            self.logger.warning(f"Расчетная TP цена {tp_price_quantized:.{self.display_precision}f} некорректна для базовой цены {base_price_for_tp:.{self.display_precision}f}. TP ордер не будет размещен.")
            return None
        ...
        try:
            ...
            # === ДОБАВЛЕНО: Логирование цены размещённого TP ордера ===
            self.logger.debug(f"{account_state.account_name_for_log}-размещен ордер TP, цена {tp_price_quantized:.{self.display_precision}f}, количество лотов {quantity}")
            # =========================================================
            return response_id
        except Exception as e:
...
# =================================================================================
    ...
    async def _place_directional_entry_orders(self, client, account_id: str, last_known_market_price: Decimal):
        """
        ...
        async with self.orders_placement_lock:
            # === ПОЛУЧАЕМ ДАННЫЕ ПОРТФЕЛЯ ДЛЯ РАСЧЕТА МАРЖИ ===
            portfolio_response = None
            total_amount_shares = Decimal('0')
            total_amount_portfolio = Decimal('0')
            try:
                portfolio_response = await client.operations.get_portfolio(account_id=account_id)
                total_amount_shares = _quotation_to_decimal(portfolio_response.total_amount_shares)
                total_amount_portfolio = _quotation_to_decimal(portfolio_response.total_amount_portfolio)
            except Exception as e:
                self.logger.error(f"[{account_state.account_name_for_log}] Ошибка получения данных портфеля для расчета маржи: {e}")
                # Если не можем получить данные, пропускаем размещение ордеров
                return

            # === РАССЧИТЫВАЕМ ДОСТУПНУЮ МАРЖУ ===
            if account_state.is_sell_account:
                current_sell_margin = abs(total_amount_shares)
                available_margin = MAX_FREE_MARGIN_RUB_FOR_SELL - current_sell_margin
                margin_type = "SELL"
                margin_limit = MAX_FREE_MARGIN_RUB_FOR_SELL
            else:
                if total_amount_shares > total_amount_portfolio:
                    current_buy_margin = total_amount_shares - total_amount_portfolio
                else:
                    current_buy_margin = Decimal('0')
                available_margin = MAX_FREE_MARGIN_RUB_FOR_BUY - current_buy_margin
                margin_type = "BUY"
                margin_limit = MAX_FREE_MARGIN_RUB_FOR_BUY

            # === НОВОЕ: Определяем базовую цену для размещения ордеров ВХОДА ===
            # ВАЖНО: Это правило применяется ТОЛЬКО для ордеров ВХОДА. Для TP ордеров всегда используется entry_price конкретного лота.
            if account_state.initial_orders_placed:
                # Сценарий 1: Начальный ордер уже был размещен (или позиция была синхронизирована).
                # Используем цену входа (последнюю или усредненную из open_lots).
                # В простейшем случае - last_position_fill_price.
                base_price_for_entry_orders = last_known_market_price
                self.logger.debug(f"[{account_state.account_name_for_log}] Размещение ордеров для входа: Используем last_position_fill_price ({base_price_for_entry_orders:.{self.display_precision}f}) как базовую цену для ордеров ВХОДА.")
            else:
                # Сценарий 2: Начальный ордер еще не размещался (чистый старт без позиции или после сброса).
                # Используем актуальную рыночную цену.
                base_price_for_entry_orders = last_known_market_price
                self.logger.debug(f"[{account_state.account_name_for_log}] Размещение ордеров для входа: Используем last_known_market_price ({base_price_for_entry_orders:.{self.display_precision}f}) как базовую цену для ордеров ВХОДА.")
            
            if base_price_for_entry_orders <= 0:
                self.logger.warning(f"[{account_state.account_name_for_log}] Базовая цена для ордеров входа {base_price_for_entry_orders:.{self.display_precision}f} некорректна. Ордера для входа не будут размещены.")
                return
            # =====================================================================

            self.logger.debug(f"[{account_state.account_name_for_log}] Размещение/обновление ордеров для входа. Базовая цена: {base_price_for_entry_orders:.{self.display_precision}f}")

            # --- Шаг 1: Отмена старых ордеров для входа ---
            # (Восстановлено из оригинального кода, заменяет несуществующий self._cancel_old_entry_orders)
            ...
            if orders_to_cancel_tasks:
                self.logger.debug(f"Для {account_state.account_name_for_log}: Отмена старых ордеров для входа (верхний: {old_upper_entry_id}, нижний: {old_lower_entry_id}) перед размещением новых.")
            ...
            # (Восстановлена и дополнена логикой проверки маржи)
            placement_tasks = []
            ...
            if account_state.is_sell_account: # SELL Account (Shorting)
                # --- Верхний ордер (SELL LIMIT) ---
                upper_price = base_price_for_entry_orders + interval 
                # Используем округление из оригинального кода
                ...
                if upper_price_quantized > 0:
                    # === ПРОВЕРКА МАРЖИ ДЛЯ ОРДЕРА ВЫШЕ (SELL LIMIT) ===
                    required_margin_upper_limit = upper_price * Decimal(LOTS)
                    self.logger.debug(f"[{account_state.account_name_for_log}] Проверка маржи для ордера ВЫШЕ (SELL LIMIT). Цена: {upper_price:.{self.display_precision}f}. Доступно: {available_margin:.{self.display_precision}f} RUB. Требуется: {required_margin_upper_limit:.{self.display_precision}f} RUB.")
                
                    if available_margin >= required_margin_upper_limit:
                        # Размещаем лимитный ордер на продажу выше
                        ...
                        self.logger.debug(f"[{account_state.account_name_for_log}] Лимит использования маржи достигнут ({margin_limit:.{self.display_precision}f} RUB). Ордер ВЫШЕ не размещается.")
                else:
                ...
                # --- Нижний ордер (SELL STOP-LOSS) ---
                lower_price = base_price_for_entry_orders - interval 
                ...
                if lower_price_quantized > 0:
                    # === ПРОВЕРКА МАРЖИ ДЛЯ ОРДЕРА НИЖЕ (SELL STOP-LOSS) ===
                    # Для стоп-ордера используем цену стопа для расчета требуемой маржи
                    required_margin_lower_stop = lower_price * Decimal(LOTS)
                    self.logger.debug(f"[{account_state.account_name_for_log}] Проверка маржи для ордера НИЖЕ (SELL STOP-LOSS). Цена: {lower_price:.{self.display_precision}f}. Доступно: {available_margin:.{self.display_precision}f} RUB. Требуется: {required_margin_lower_stop:.{self.display_precision}f} RUB.")
                
                    if available_margin >= required_margin_lower_stop:
                        # Размещаем стоп-лосс ордер на продажу ниже
                        lower_order_future = self.place_stop_order(
                            ...
                            limit_price=None, # SL обычно рыночный ордер при срабатывании стопа
                            order_tag="SELL_ENTRY_LOWER_STOP"
                        )
                        placement_tasks.append(lower_order_future)
                    else:
                        self.logger.debug(f"[{account_state.account_name_for_log}] Лимит использования маржи достигнут ({margin_limit:.{self.display_precision}f} RUB). Ордер НИЖЕ не размещается.")
                else:
                ...
                # --- Нижний ордер (BUY LIMIT) ---
                lower_price = base_price_for_entry_orders - interval 
                lower_price_quantized = (lower_price / self.min_price_increment).quantize(Decimal('1.'), rounding=ROUND_HALF_UP) * self.min_price_increment
                if lower_price_quantized > 0:
                    # === ПРОВЕРКА МАРЖИ ДЛЯ ОРДЕРА НИЖЕ (BUY LIMIT) ===
                    required_margin_lower_limit = lower_price * Decimal(LOTS)
                    self.logger.debug(f"[{account_state.account_name_for_log}] Проверка маржи для ордера НИЖЕ (BUY LIMIT). Цена: {lower_price:.{self.display_precision}f}. Доступно: {available_margin:.{self.display_precision}f} RUB. Требуется: {required_margin_lower_limit:.{self.display_precision}f} RUB.")
                
                    if available_margin >= required_margin_lower_limit:
                        # Размещаем лимитный ордер на покупку ниже
                        ...
                        self.logger.warning(f"[{account_state.account_name_for_log}] Лимит использования маржи достигнут ({margin_limit:.{self.display_precision}f} RUB). Ордер НИЖЕ не размещается.")
                else:
                ...
                # --- Верхний ордер (BUY STOP-LOSS) ---
                upper_price = base_price_for_entry_orders + interval 
                upper_price_quantized = (upper_price / self.min_price_increment).quantize(Decimal('1.'), rounding=ROUND_HALF_UP) * self.min_price_increment
                if upper_price_quantized > 0:
                    # === ПРОВЕРКА МАРЖИ ДЛЯ ОРДЕРА ВЫШЕ (BUY STOP-LOSS) ===
                    required_margin_upper_stop = upper_price * Decimal(LOTS)
                    self.logger.debug(f"[{account_state.account_name_for_log}] Проверка маржи для ордера ВЫШЕ (BUY STOP-LOSS). Цена: {upper_price:.{self.display_precision}f}. Доступно: {available_margin:.{self.display_precision}f} RUB. Требуется: {required_margin_upper_stop:.{self.display_precision}f} RUB.")
                
                    if available_margin >= required_margin_upper_stop:
                        # Размещаем стоп-лосс ордер на покупку выше
                        ...
                        self.logger.warning(f"[{account_state.account_name_for_log}] Лимит использования маржи достигнут ({margin_limit:.{self.display_precision}f} RUB). Ордер ВЫШЕ не размещается.")
                else:
            ...
            if placement_tasks:
                ...
                if account_state.is_sell_account:
                    if upper_response_id:
                        # Для SELL: верхний ордер - это лимитный (False)
                        account_state.active_entry_order_above = (upper_response_id, False)
                        # Логирование размещения уже внутри place_limit_order/place_stop_order
                    else:
                    ...
                    if lower_response_id:
                        # Для SELL: нижний ордер - это стоп (True)
                    ...
                    if lower_response_id:
                        # Для BUY: нижний ордер - это лимитный (False)
                    ...
                    if upper_response_id:
                        # Для BUY: верхний ордер - это стоп (True)
                ...
                self.logger.debug(f"Для {account_state.account_name_for_log}: Нет ордеров для размещения.")
                            ...
                            if od.order_id == api_found_stop_order.stop_order_id:
                                ...
                                if api_found_stop_order.exchange_order_id and not identified_order_data.exchange_order_id:
                                    ...
                                    if api_found_stop_order.status == StopOrderStatusOption.STOP_ORDER_STATUS_EXECUTED:
                                        ...
                                        self.logger.debug(f"[{account_state.account_name_for_log}] Идентификация: Стоп-ордер {identified_order_data.order_id} (API Sync) исполнен. Биржевой ID: {api_found_stop_order.exchange_order_id}.")
                                    else: # Must be active
    ...
    async def _process_opening_order_fill(self, client, account_id: str, order_trades_data: OrderTrades, filled_trade_item: OrderTrade, effective_order_id_for_processing: str, order_tag: str):
            ...
            #Размещает TP ордер, используя зафиксированную рыночную цену для согласованности сетки.
            ...
            # === НОВОЕ: Защита критической секции с помощью asyncio.Lock ===
            # Вся логика внутри этого блока будет выполняться только одной корутиной
            # для данного account_state в любой момент времени.
            async with account_state._state_lock:
                # ==============================================================

                filled_price_decimal = _quotation_to_decimal(filled_trade_item.price)
                filled_lots = filled_trade_item.quantity

                # === НОВОЕ: Фиксация рыночной цены для согласованного расчета TP и входных ордеров ===
                # Используем last_known_market_price, предполагая, что она обновляется стримом рыночных данных
                base_price_for_this_fill_event = self.last_known_market_price
                if not base_price_for_this_fill_event or base_price_for_this_fill_event == Decimal('0'):
                    self.logger.warning(f"[{account_state.account_name_for_log}] _process_opening_order_fill: last_known_market_price недоступна. Используем filled_price_decimal ({filled_price_decimal}) как запасной вариант для TP.")
                    base_price_for_this_fill_event = filled_price_decimal
                self.logger.debug(f"[{account_state.account_name_for_log}] _process_opening_order_fill: Зафиксирована цена для TP и последующей перестановки: {base_price_for_this_fill_event}")
                # ================================================================================

                new_lot = {'entry_price': filled_price_decimal, 'tp_order_id': None, 'quantity': filled_lots}
                account_state.open_lots.append(new_lot)

                # === ИЗМЕНЕНО: Передаем зафиксированную цену в _place_individual_take_profit_order ===
                # Вместо filled_price_decimal для расчета TP, используем base_price_for_this_fill_event
                tp_order_id = await self._place_individual_take_profit_order(client, account_id, base_price_for_this_fill_event, filled_lots)
                # ================================================================================
                if tp_order_id:
                    new_lot['tp_order_id'] = tp_order_id

                # === ИЗМЕНЕНО: last_position_fill_price по-прежнему устанавливается на filled_price_decimal ===
                # Это нужно для корректной работы логики PnL и, возможно, других частей бота.
                # Но для расчета сетки (входные ордера и TP) мы используем base_price_for_this_fill_event.
                account_state.last_position_fill_price = filled_price_decimal # Оригинальная логика
                # ================================================================================
                ...
                # (остается без изменений, но теперь защищена блокировкой)
                ...
                # Сохранение состояния внутри блокировки
                self.save_state()
...
# =================================================================================
    ...
    async def _process_tp_order_fill(self, client, account_id: str, effective_order_id_for_processing: str, filled_price_decimal: Decimal, filled_lots: int):
        ...
        Вместо удаления целого лота, уменьшает его количество (quantity).
        """
        account_state = self.account_states.get(account_id)
        if not account_state:
        ...
        # === НОВОЕ: Используем asyncio.Lock для предотвращения race condition ===
        async with account_state._state_lock:
        # ==============================================================

            # --- Логика уменьшения количества в лоте ---
            lots_to_process = account_state.open_lots
            updated_lots = []
            lot_found_and_updated = False
            total_units_closed = 0

            for lot in lots_to_process:
                if not lot_found_and_updated and lot.get('tp_order_id') == effective_order_id_for_processing:
                    # Найден лот, связанный с исполненным TP ордером
                    lot_quantity = lot['quantity']
                    
                    # Определяем, сколько единиц было закрыто этим TP ордером
                    # Для простоты предполагаем, что TP закрывает все единицы лота
                    # (можно усложнить, если TP может закрывать часть лота)
                    units_closed_by_this_tp = min(filled_lots, lot_quantity) 
                    total_units_closed += units_closed_by_this_tp
                    
                    self.logger.debug(f"[{account_state.account_name_for_log}] TP ордер {effective_order_id_for_processing} связан с лотом на {lot_quantity} ед. Закрываем {units_closed_by_this_tp} ед.")

                    if units_closed_by_this_tp == lot_quantity:
                        # Закрывается весь лот
                        self.logger.info(f"[{account_state.account_name_for_log}] TP ордер {effective_order_id_for_processing}: Закрыт весь лот на {lot_quantity} ед. по цене входа {lot['entry_price']:.{self.display_precision}f}. Лот удален.")
                        # Лот не добавляется в updated_lots, эффективно удаляя его
                        lot_found_and_updated = True # Предполагаем, что один TP ордер связан с одним лотом
                        
                        # --- Обновление PnL ---
                        closed_lot_entry_price = lot['entry_price']
                        pnl = (closed_lot_entry_price - filled_price_decimal) * units_closed_by_this_tp * account_state.lots_in_one_lot if account_state.is_sell_account else \
                              (filled_price_decimal - closed_lot_entry_price) * units_closed_by_this_tp * account_state.lots_in_one_lot
                        ...
                        # Закрывается часть лота
                        new_lot_quantity = lot_quantity - units_closed_by_this_tp
                        lot['quantity'] = new_lot_quantity
                        updated_lots.append(lot)
                        self.logger.info(f"[{account_state.account_name_for_log}] TP ордер {effective_order_id_for_processing}: Закрыто {units_closed_by_this_tp} ед. из лота. Осталось {new_lot_quantity} ед.")
                        lot_found_and_updated = True # Предполагаем, что один TP ордер связан с одним лотом
                        
                        # --- Обновление PnL для части лота ---
                        closed_lot_entry_price = lot['entry_price']
                        pnl = (closed_lot_entry_price - filled_price_decimal) * units_closed_by_this_tp * account_state.lots_in_one_lot if account_state.is_sell_account else \
                              (filled_price_decimal - closed_lot_entry_price) * units_closed_by_this_tp * account_state.lots_in_one_lot
                        account_state.daily_profit_loss += pnl
                    ...
                    # Лот не связан с этим TP ордером, добавляем как есть
                    updated_lots.append(lot)

            if not lot_found_and_updated:
                self.logger.warning(f"[{account_state.account_name_for_log}] TP ордер {effective_order_id_for_processing} исполнен, но не найден связанный лот в open_lots. Возможно, лот уже был закрыт другим способом или это старый ордер.")

            # Обновляем список лотов в состоянии аккаунта
            account_state.open_lots = updated_lots

            # --- Логика после закрытия позиции ---
            self.logger.info(f"[{account_state.account_name_for_log}] После срабатывания TP исполнено/закрыто {total_units_closed} единиц. Текущая позиция: {sum(l['quantity'] for l in account_state.open_lots)} лот(ов).")

            if not account_state.open_lots:
                self.logger.info(f"[{account_state.account_name_for_log}] Позиция полностью закрыта через TP.")
                ...
                # Если позиция не пуста, устанавливаем флаг для перестановки ордеров входа
                # Это важно, потому что количество лотов изменилось
                account_state.orders_need_reposition = True
    ...
    async def handle_order_fill(self, client, order_trades_data: OrderTrades, filled_trade_item: OrderTrade):
        ...
        self.logger.debug(f"Позиция {account_state.account_name_for_log} обновлена: {current_position_lots} лот(ов) по средней цене {average_position_price:.{self.display_precision}f}")
    ...
    async def _check_margin_and_manage_orders(self, client, account_id: str):
        """Проверяет текущее использование маржи (новая логика на основе GetPortfolio) и устанавливает флаг margin_limit_hit."""
        account_state = self.account_states.get(account_id)
        if not account_state:
        ...
        try:
            # === НОВАЯ ЛОГИКА РАСЧЕТА МАРЖИ НА ОСНОВЕ GetPortfolio ===
            # Получаем данные портфеля
            portfolio_response = await client.operations.get_portfolio(account_id=account_id)
            total_amount_shares = _quotation_to_decimal(portfolio_response.total_amount_shares)
            total_amount_portfolio = _quotation_to_decimal(portfolio_response.total_amount_portfolio)
        
            # === ИСПРАВЛЕННЫЙ РАСЧЕТ ТРЕБУЕМОЙ МАРЖИ ДЛЯ НОВЫХ ОРДЕРОВ ===
            # Используем last_known_market_price и GRID_INTERVAL для расчета
            base_price = account_state.last_known_market_price if hasattr(account_state, 'last_known_market_price') and account_state.last_known_market_price else Decimal('100') # fallback
            # Расчет требуемой маржи: цена потенциального входного ордера * количество лотов
            # Для BUY: цена входа ниже (base_price - GRID_INTERVAL). Для SELL: цена входа выше (base_price + GRID_INTERVAL)
            # Для консервативной оценки используем базовую цену
            required_margin_for_new_orders = base_price * Decimal(LOTS)
        
            if account_state.is_sell_account:
                # Расчет для SELL счета
                current_sell_margin = abs(total_amount_shares)
                available_sell_margin = MAX_FREE_MARGIN_RUB_FOR_SELL - current_sell_margin
            
                #self.logger.info(f"[{account_state.account_name_for_log}] ДОСТУПНАЯ МАРЖА для SELL = {available_sell_margin:.{self.display_precision}f} RUB. Требуется для новых ордеров: {required_margin_for_new_orders:.{self.display_precision}f} RUB.")
            
                # Проверка превышения лимита
                if available_sell_margin < required_margin_for_new_orders:
                    account_state.margin_limit_hit = True
                    #self.logger.warning(f"[{account_state.account_name_for_log}] Доступная маржа ({available_sell_margin:.{self.display_precision}f} RUB) меньше требуемой ({required_margin_for_new_orders:.{self.display_precision}f} RUB).")
                else:
                    account_state.margin_limit_hit = False
            else:
                # Расчет для BUY счета
                if total_amount_shares > total_amount_portfolio:
                    current_buy_margin = total_amount_shares - total_amount_portfolio
                else:
                    current_buy_margin = Decimal('0')
            
                available_buy_margin = MAX_FREE_MARGIN_RUB_FOR_BUY - current_buy_margin
            
               # self.logger.info(f"[{account_state.account_name_for_log}] ДОСТУПНАЯ МАРЖА для BUY = {available_buy_margin:.{self.display_precision}f} RUB. Требуется для новых ордеров: {required_margin_for_new_orders:.{self.display_precision}f} RUB.")
            
                # Проверка превышения лимита
                if available_buy_margin < required_margin_for_new_orders:
                    account_state.margin_limit_hit = True
                    #self.logger.warning(f"[{account_state.account_name_for_log}] Доступная маржа ({available_buy_margin:.{self.display_precision}f} RUB) меньше требуемой ({required_margin_for_new_orders:.{self.display_precision}f} RUB).")
                else:
                    account_state.margin_limit_hit = False
    ...
    async def _perform_end_of_session_margin_reduction(self, client, account_id: str, last_known_market_price: Decimal):
        ...
        if now.time() >= account_state.end_of_session_reduction_time and now.time() <= EXT_TRADING_END_TIME:
            ...
            try:
                # === НОВАЯ ЛОГИКА РАСЧЕТА МАРЖИ ДЛЯ СОКРАЩЕНИЯ ===
                # Получаем данные портфеля
                portfolio_response = await client.operations.get_portfolio(account_id=account_id)
                total_amount_shares = _quotation_to_decimal(portfolio_response.total_amount_shares)
                total_amount_portfolio = _quotation_to_decimal(portfolio_response.total_amount_portfolio)
                ...
                if account_state.is_sell_account:
                    # Расчет для SELL счета
                    current_sell_margin = abs(total_amount_shares)
                    self.logger.info(f"[{account_state.account_name_for_log}] Текущая маржа SELL: {current_sell_margin:.{self.display_precision}f} RUB. Лимит: {MAX_FREE_MARGIN_RUB_FOR_SELL:.{self.display_precision}f} RUB.")
                
                    # Проверка превышения лимита для сокращения
                    if current_sell_margin > MAX_FREE_MARGIN_RUB_FOR_SELL:
                        margin_exceeded = True
                        self.logger.warning(f"[{account_state.account_name_for_log}] Маржа ({current_sell_margin:.{self.display_precision}f} RUB) превышает лимит ({MAX_FREE_MARGIN_RUB_FOR_SELL:.{self.display_precision}f} RUB). Начинаем сокращение позиции.")
                else:
                    # Расчет для BUY счета
                    if total_amount_shares > total_amount_portfolio:
                        current_buy_margin = total_amount_shares - total_amount_portfolio
                    ...
                    self.logger.info(f"[{account_state.account_name_for_log}] Текущая маржа BUY: {current_buy_margin:.{self.display_precision}f} RUB. Лимит: {MAX_FREE_MARGIN_RUB_FOR_BUY:.{self.display_precision}f} RUB.")
                
                    # Проверка превышения лимита для сокращения
                    if current_buy_margin > MAX_FREE_MARGIN_RUB_FOR_BUY:
                        margin_exceeded = True
                        self.logger.warning(f"[{account_state.account_name_for_log}] Маржа ({current_buy_margin:.{self.display_precision}f} RUB) превышает лимит ({MAX_FREE_MARGIN_RUB_FOR_BUY:.{self.display_precision}f} RUB). Начинаем сокращение позиции.")
            
                if margin_exceeded:
                    # Отменяем только активные ордера для входа через OrderManager
                    await self.cancel_all_orders_for_account(client, account_id, account_state.account_name_for_log, cancel_entry_orders_only=True)
                
                    # Получаем текущую позицию из портфеля
                    portfolio = await client.operations.get_portfolio(account_id=account_id)
                    current_figi_position = next((pos for pos in portfolio.positions if pos.figi == self.figi), None)
                
                    if current_figi_position and current_figi_position.quantity:
                        current_position_qty = _quotation_to_decimal(current_figi_position.quantity)
                    
                        if current_position_qty > 0:
                            # Определяем направление закрытия
                            direction = OrderDirection.ORDER_DIRECTION_SELL if not account_state.is_sell_account else OrderDirection.ORDER_DIRECTION_BUY
                        
                            # Размещаем рыночный ордер на закрытие части позиции
                            # Для простоты закрываем 1 лот (можно адаптировать логику)
                            lots_to_close = min(Decimal(LOTS), current_position_qty)
                        
                            self.logger.info(f"[{account_state.account_name_for_log}] Размещаем рыночный ордер на закрытие {lots_to_close} лот(ов).")
                        
                            market_order_response = await client.orders.post_order(
                                figi=self.figi,
                                quantity=int(lots_to_close),
                                direction=direction,
                                account_id=account_id,
                                order_type=OrderType.ORDER_TYPE_MARKET,
                            ...
                            market_order_id = market_order_response.order_id
                        
                            # Ждем исполнения ордера
                            order_data = await self._wait_for_order_execution(client, account_id, market_order_id)
                        
                            if order_data and order_data.status == OrderExecutionReportStatus.EXECUTION_REPORT_STATUS_FILL:
                                ...
                                # Пересчитываем маржу после закрытия
                                portfolio_response = await client.operations.get_portfolio(account_id=account_id)
                                total_amount_shares = _quotation_to_decimal(portfolio_response.total_amount_shares)
                                total_amount_portfolio = _quotation_to_decimal(portfolio_response.total_amount_portfolio)
                                ...
                                if account_state.is_sell_account:
                                    ...
                                    if current_sell_margin <= MAX_FREE_MARGIN_RUB_FOR_SELL:
                                        ...
                                        self.save_state()
                                        self.logger.info(f"[{account_state.account_name_for_log}] Сокращение позиции завершено. Маржа в пределах лимита.")
                                        return
                                    else:
                                        self.logger.info(f"[{account_state.account_name_for_log}] После закрытия маржа ({current_sell_margin:.{self.display_precision}f} RUB) все еще превышает лимит ({MAX_FREE_MARGIN_RUB_FOR_SELL:.{self.display_precision}f} RUB). Продолжаем сокращение.")
                                else:
                                    if total_amount_shares > total_amount_portfolio:
                                        current_buy_margin = total_amount_shares - total_amount_portfolio
                                    else:
                                    ...
                                    if current_buy_margin <= MAX_FREE_MARGIN_RUB_FOR_BUY:
                                        ...
                                        self.logger.info(f"[{account_state.account_name_for_log}] После закрытия маржа ({current_buy_margin:.{self.display_precision}f} RUB) все еще превышает лимит ({MAX_FREE_MARGIN_RUB_FOR_BUY:.{self.display_precision}f} RUB). Продолжаем сокращение.")
                            else:
                        ...
                        # Нет позиции для сокращения
                        account_state.end_of_session_reduction_done = True
                        self.save_state()
                        self.logger.info(f"[{account_state.account_name_for_log}] Нет позиции для сокращения. Считаем сокращение завершенным.")
                        return
                    
                else:
                    self.logger.info(f"[{account_state.account_name_for_log}] Маржа в пределах лимита. Сокращение позиции не требуется.")
                    account_state.end_of_session_reduction_done = True
                    self.save_state()
    ...
    async def _sync_position_with_api(self, client, account_id: str, current_lots_held_from_api: Decimal, current_avg_price_from_api: Decimal, last_known_market_price: Decimal):
        ...
        Упрощена логика размещения TP для предотвращения зацикливания.
        """
        account_state = self.account_states.get(account_id)
        if not account_state:
        ...
        # === НОВОЕ: Проверка на частые вызовы синхронизации ===
        # (Этот блок остается без изменений)
        current_time = datetime.now()
        sync_flag_attr = f"_last_sync_time_{account_id}"
        last_sync_time = getattr(self, sync_flag_attr, None)
        
        if last_sync_time:
            time_since_last_sync = current_time - last_sync_time
            # Ограничиваем частоту синхронизации до 1 раза в секунду
            if time_since_last_sync < timedelta(seconds=1):
                self.logger.debug(f"[{account_state.account_name_for_log}] Пропущена частая синхронизация (прошло {(time_since_last_sync).total_seconds():.2f} секунд).")
                return
        
        # Устанавливаем время последней синхронизации
        setattr(self, sync_flag_attr, current_time)
        # ======================================================
        ...
        try:
            # Получаем все стоп-ордера разных статусов
        ...
        # ==========================================================================
        # === НОВАЯ ЛОГИКА: Разбитая синхронизация ===
        # ==========================================================================
        
        # --- Шаг 3: Реконсиляция внутреннего списка open_lots с API позицией ---
        # СОХРАНЯЕМ КОПИЮ текущего состояния для проверки TP
        previous_open_lots_snapshot = list(account_state.open_lots) # Создаем копию списка
        ...
        internal_total_quantity = sum(l['quantity'] for l in reconciled_open_lots) # Пересчитываем после валидации
        
        # --- Фаза 2: Обработка расхождений в количестве лотов ---
        if int(current_lots_held_from_api) > internal_total_quantity:
            ...
            new_units_to_add_count = int(current_lots_held_from_api) - internal_total_quantity
            self.logger.info(f"[{account_state.account_name_for_log}] SYNC: Обнаружено {new_units_to_add_count} новых единиц(ы) по API. Добавляем во внутреннее состояние.")
            # ИСПРАВЛЕНО: Добавляем один лот с количеством равным недостающим единицам
            reconciled_open_lots.append({'entry_price': last_known_market_price, 'tp_order_id': None, 'quantity': new_units_to_add_count})
            ...
            # Единицы были закрыты извне (например, вручную)
            # ИСПРАВЛЕНО: Уменьшаем количество единиц, а не удаляем целые лоты
            units_to_remove_count = internal_total_quantity - int(current_lots_held_from_api)
            self.logger.warning(f"[{account_state.account_name_for_log}] SYNC: Обнаружено {units_to_remove_count} единиц(ы), закрытых извне. Уменьшаем количество в лотах.")
            
            # Уменьшаем количество единиц, а не удаляем целые лоты
            remaining_units_to_remove = units_to_remove_count
            new_reconciled_lots = []
            
            for lot in reconciled_open_lots: # <-- Итерируемся по текущему списку
                if remaining_units_to_remove <= 0:
                    # Если уже убрали всё, что нужно, добавляем лот как есть
                    new_reconciled_lots.append(lot)
                else:
                    lot_quantity = lot['quantity']
                    if lot_quantity <= remaining_units_to_remove:
                        # Удаляем весь лот (все его единицы)
                        remaining_units_to_remove -= lot_quantity
                        self.logger.debug(f"[{account_state.account_name_for_log}] SYNC: Удален лот на {lot_quantity} единиц.")
                        # Если у лота был TP, он будет потерян, что может быть правильно, 
                        # если весь лот закрыт. Но нужно быть осторожным.
                        # Возможно, стоит добавить проверку на наличие TP.
                    else:
                        # Уменьшаем количество в лоте
                        lot['quantity'] = lot_quantity - remaining_units_to_remove
                        remaining_units_to_remove = 0
                        new_reconciled_lots.append(lot)
                        self.logger.debug(f"[{account_state.account_name_for_log}] SYNC: Уменьшено количество в лоте с {lot_quantity} до {lot['quantity']} единиц.")
            
            reconciled_open_lots = new_reconciled_lots
            ...
            self.logger.debug(f"[{account_state.account_name_for_log}] SYNC: last_position_fill_price обновлена до рыночной цены ({account_state.last_position_fill_price:.{self.display_precision}f}) из-за единиц, закрытых извне.")
        
        # ВАЖНО: Обновляем account_state.open_lots ДО шага 4
        account_state.open_lots = reconciled_open_lots
        self.logger.debug(f"[{account_state.account_name_for_log}] SYNC: Обновленный open_lots: {account_state.open_lots}")

        # --- Шаг 4: УЛУЧШЕННАЯ Синхронизация TP ордеров ---
        # ВАЖНО: Этот шаг должен идти ПОСЛЕ шага 3, где account_state.open_lots обновляется на reconciled_open_lots
        self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: Начинаем улучшенную синхронизацию TP ордеров.")

        # --- 4a. Попытка восстановления TP для старых лотов ---
        # Ищем лоты в ОБНОВЛЕННОМ списке, у которых tp_order_id стал None
        lots_for_recovery_tp = [lot for lot in account_state.open_lots if lot.get('tp_order_id') is None]
        
        # --- 4b. Размещение TP для восстановления ---
        if lots_for_recovery_tp:
            self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: Нужно восстановить TP для {len(lots_for_recovery_tp)} групп(ы) лотов.")
            recovery_tasks = []
            for lot in lots_for_recovery_tp:
                # Используем параметры САМОГО ЛОТА для восстановления TP
                recovery_price = lot['entry_price']
                recovery_quantity = lot['quantity']
                
                # Создаем задачу на размещение TP для этой группы
                task = self._place_individual_take_profit_order(
                    client, account_id, recovery_price, recovery_quantity
                )
                recovery_tasks.append((task, lot)) # Сохраняем лот, чтобы потом обновить его tp_order_id
            
            # Выполняем все задачи восстановления
            if recovery_tasks:
                task_results = await asyncio.gather(*(task for task, _ in recovery_tasks), return_exceptions=True)
                # Обрабатываем результаты
                for i, (result, lot) in enumerate(zip(task_results, (lot for _, lot in recovery_tasks))):
                    if not isinstance(result, Exception) and result:
                        lot['tp_order_id'] = result # Обновляем tp_order_id в reconciled_open_lots
                        # Нужно также обновить его в основном account_state.open_lots, 
                        # но account_state.open_lots уже обновлен до reconciled_open_lots.
                        # Это будет сделано после шага 4.
                        self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: Восстановлен TP ордер {result} на {lot['quantity']} ед. по цене {lot['entry_price']:.{self.display_precision}f}.")
                    else:
                        self.logger.error(f"[{account_state.account_name_for_log}] SYNC_TP: НЕ УДАЛОСЬ восстановить TP для {lot['quantity']} ед. по цене {lot['entry_price']:.{self.display_precision}f}. Ошибка: {result}")
        ...
        # Обновляем основной список лотов
        # ВАЖНО: Это уже сделано выше ПЕРЕД шагом 4
        # account_state.open_lots = reconciled_open_lots
...
# =================================================================================
    ...
    async def _reconcile_open_lots_with_api(self, client: AsyncClient, account_state: AccountStateData, 
                                             current_lots_held_from_api: Decimal, last_known_market_price: Decimal,
                                             active_exchange_stop_orders: list, executed_stop_orders: list, canceled_stop_orders: list) -> List[Dict[str, Any]]:
        """
        Реконсилирует внутренний список open_lots с API позицией и активными TP.
        Возвращает новый список лотов (reconciled_open_lots).
        """
        self.logger.debug(f"[{account_state.account_name_for_log}] RECONCILE: Начинаем реконсиляцию open_lots с API.")
        ...
        for lot in account_state.open_lots:
            ...
            if tp_id:
                ...
                # Проверяем, был ли TP исполнен или отменен
                     ...
                     any(so.stop_order_id == tp_id for so in canceled_stop_orders):
                    self.logger.debug(f"[{account_state.account_name_for_log}] RECONCILE: TP ордер {tp_id} для лота {lot['entry_price']:.{self.display_precision}f} неактивен на бирже (исполнен/отменен). Сбрасываем ID TP.")
                    new_lot = lot.copy()
                    new_lot['tp_order_id'] = None # Сбрасываем ID TP ордера
                    reconciled_open_lots.append(new_lot)
                    # Обновим статус в OrderManager
                    ...
                    # TP не найден ни в активных, ни в исполненных, ни в отмененных. Считаем, что его нет.
                    self.logger.info(f"[{account_state.account_name_for_log}] RECONCILE: TP ордер {tp_id} для лота {lot['entry_price']:.{self.display_precision}f} не найден на бирже ни в каком статусе. Сбрасываем ID TP.")
                    new_lot = lot.copy()
                    new_lot['tp_order_id'] = None
                    reconciled_open_lots.append(new_lot)
                    order_data_in_manager = self.get_order_data(tp_id)
        ...
        if int(current_lots_held_from_api) > internal_total_quantity:
            # Обнаружены новые лоты (сработал ордер на вход, пока бот был неактивен)
            new_units_to_add_count = int(current_lots_held_from_api) - internal_total_quantity
            self.logger.info(f"[{account_state.account_name_for_log}] RECONCILE: Обнаружено {new_units_to_add_count} новых единиц(ы) по API. Добавляем во внутреннее состояние.")
            # ИСПРАВЛЕНО: Добавляем один лот с количеством равным недостающим единицам
            reconciled_open_lots.append({'entry_price': last_known_market_price, 'tp_order_id': None, 'quantity': new_units_to_add_count})
            
            # CRITICAL: If new lots were added via sync, force last_position_fill_price to current market price
            account_state.last_position_fill_price = last_known_market_price
            self.logger.debug(f"[{account_state.account_name_for_log}] RECONCILE: last_position_fill_price обновлена до рыночной цены ({account_state.last_position_fill_price:.{self.display_precision}f}) из-за новых лотов, обнаруженных синхронизацией.")

        elif int(current_lots_held_from_api) < internal_total_quantity:
            # Единицы были закрыты извне (например, вручную)
            # ИСПРАВЛЕНО: Уменьшаем количество единиц, а не удаляем целые лоты
            units_to_remove_count = internal_total_quantity - int(current_lots_held_from_api)
            self.logger.warning(f"[{account_state.account_name_for_log}] RECONCILE: Обнаружено {units_to_remove_count} единиц(ы), закрытых извне. Уменьшаем количество в лотах.")
            ...
            for lot in reconciled_open_lots: # <-- Итерируемся по текущему списку
                    if remaining_units_to_remove <= 0:
                        ...
                        lot_quantity = lot['quantity']
                        # --- НОВОЕ: Логика отмены TP перед удалением/уменьшением ---
                        tp_id_to_cancel = lot.get('tp_order_id')
                        if tp_id_to_cancel:
                            self.logger.info(f"[{account_state.account_name_for_log}] RECONCILE: Пытаемся отменить TP ордер {tp_id_to_cancel}, связанный с удаляемым/уменьшаемым лотом (количество {lot_quantity} -> {lot_quantity - min(lot_quantity, remaining_units_to_remove)}).")
                            # Инициируем отмену асинхронно, не дожидаясь результата (fire-and-forget)
                            # Это предотвращает блокировку основного потока синхронизации
                            # и обрабатывает случай, когда ордер уже исполнен/отменен
                            try:
                                # asyncio.create_task(self.cancel_order(client, tp_id_to_cancel, is_stop_order=True, account_id=account_state.account_id))
                                # Используем ensure_future для большей совместимости
                                asyncio.ensure_future(self.cancel_order(client, tp_id_to_cancel, is_stop_order=True, account_id=account_state.account_id))
                            except Exception as e:
                                 self.logger.warning(f"[{account_state.account_name_for_log}] RECONCILE: Исключение при попытке асинхронной отмены TP {tp_id_to_cancel}: {e}")
                            # Сбрасываем tp_order_id в лоте, чтобы избежать повторной попытки отмены
                            # и корректной работы последующей логики SYNC_TP
                        ...
                        if lot_quantity <= remaining_units_to_remove:
                            # Удаляем весь лот (все его единицы)
                            remaining_units_to_remove -= lot_quantity
                            self.logger.debug(f"[{account_state.account_name_for_log}] RECONCILE: Удален лот на {lot_quantity} единиц.")
                            # TP уже отменен выше
                        else:
                            # Уменьшаем количество в лоте
                            lot['quantity'] = lot_quantity - remaining_units_to_remove
                            remaining_units_to_remove = 0
                            new_reconciled_lots.append(lot)
                            self.logger.debug(f"[{account_state.account_name_for_log}] RECONCILE: Уменьшено количество в лоте с {lot_quantity} до {lot['quantity']} единиц.")
                            # TP уже отменен выше. SYNC_TP позже решит, нужно ли восстанавливать/пересоздавать TP для нового количества.
                
            reconciled_open_lots = new_reconciled_lots # <-- Обновляем список лотов    
            
            # CRITICAL: If lots were removed, the base price for repositioning should be the current market price,
            # as the old average value might be incorrect or the position significantly changed.
            account_state.last_position_fill_price = last_known_market_price
            account_state.orders_need_reposition = True # Force reposition after external closure
            self.logger.debug(f"[{account_state.account_name_for_log}] RECONCILE: last_position_fill_price обновлена до рыночной цены ({account_state.last_position_fill_price:.{self.display_precision}f}) из-за единиц, закрытых извне.")
        
        self.logger.debug(f"[{account_state.account_name_for_log}] RECONCILE: Обновленный open_lots: {reconciled_open_lots}")
        return reconciled_open_lots
    

    async def _sync_take_profit_orders_after_reconciliation(self, client: AsyncClient, account_state: AccountStateData, 
                                                             reconciled_open_lots: List[Dict[str, Any]],account_id, last_known_market_price: Decimal):
        """
        Синхронизирует TP ордера после реконсиляции лотов.
        Восстанавливает TP для существующих лотов и размещает для новых.
        """
        self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: Начинаем улучшенную синхронизацию TP ордеров.")

        # --- 4a. Попытка восстановления TP для старых лотов ---
        # Мы хотим восстановить TP для тех лотов, которые существовали в previous_open_lots_snapshot,
        # но их TP теперь не активен (tp_order_id сброшен в reconciled_open_lots).
        
        lots_for_recovery_tp = [] # Лоты для восстановления старых TP
        lots_for_new_tp = []      # Лоты для новых TP (например, увеличение позиции)
        
        # Для сопоставления создадим вспомогательную структуру из previous_open_lots_snapshot
        # Ключ: (entry_price, quantity), Значение: сам лот
        old_lots_map = {(lot['entry_price'], lot['quantity']): lot for lot in account_state.open_lots if lot.get('tp_order_id')} # Используем состояние ДО реконсиляции
        
        # Также почитаем общее количество единиц, которое было ПОКРЫТО TP в previous_open_lots_snapshot
        expected_units_covered_by_old_tp = sum(lot['quantity'] for lot in account_state.open_lots if lot.get('tp_order_id'))
        self.logger.debug(f"[{account_state.account_name_for_log}] SYNC_TP: Ожидаемое количество единиц с TP в прошлом состоянии: {expected_units_covered_by_old_tp}")

        # Пройдемся по reconciled_open_lots (итоговое состояние после шага 3)
        for reconciled_lot in reconciled_open_lots:
            current_tp_id = reconciled_lot.get('tp_order_id')
            entry_price = reconciled_lot['entry_price']
            quantity = reconciled_lot['quantity']
            
            # Если у лота НЕТ tp_order_id, он нуждается в TP
            if not current_tp_id:
                # Проверим, существовал ли лот с такими параметрами и TP в прошлом состоянии
                if (entry_price, quantity) in old_lots_map:
                    # Это лот, который существовал раньше и имел TP, но TP стал неактивен
                    # => его нужно "восстановить"
                    lots_for_recovery_tp.append(reconciled_lot)
                    self.logger.debug(f"[{account_state.account_name_for_log}] SYNC_TP: Лот для ВОССТАНОВЛЕНИЯ TP: {quantity} ед. по цене {entry_price:.{self.display_precision}f}.")
                    # Удаляем из карты, чтобы не использовать его повторно
                    del old_lots_map[(entry_price, quantity)] 
                else:
                    # Это новый лот (появился при синхронизации позиции) или лот, который не имел TP в прошлый раз
                    # => для него применим упрощенную логику позже
                    lots_for_new_tp.append(reconciled_lot)
                    self.logger.debug(f"[{account_state.account_name_for_log}] SYNC_TP: Лот для НОВОГО TP: {quantity} ед. по цене {entry_price:.{self.display_precision}f}.")
        ...
        if lots_for_recovery_tp:
            ...
            if recovery_tasks:
                ...
                for i, (result, lot) in enumerate(zip(task_results, (lot for _, lot in recovery_tasks))):
                    if not isinstance(result, Exception) and result:
                        lot['tp_order_id'] = result # Обновляем tp_order_id в reconciled_open_lots
                        # Обновляем ID в основном списке account_state.open_lots (он же lot)
                        # (reconciled_open_lots это список словарей, изменения отражаются)
                        self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: Восстановлен TP ордер {result} на {lot['quantity']} ед. по цене {lot['entry_price']:.{self.display_precision}f}.")
                    else:
                        self.logger.error(f"[{account_state.account_name_for_log}] SYNC_TP: НЕ УДАЛОСЬ восстановить TP для {lot['quantity']} ед. по цене {lot['entry_price']:.{self.display_precision}f}. Ошибка: {result}")

        # --- 4c. Размещение TP по упрощенной логике для оставшихся ---
        # Подсчитываем общее количество единиц, для которых нужно разместить TP по упрощенной логике
        units_for_new_tp_total = sum(lot['quantity'] for lot in lots_for_new_tp)
        
        if units_for_new_tp_total > 0:
            self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: Нужно разместить TP ордер(а) по упрощенной логике на {units_for_new_tp_total} единиц.")
            
            # Используем цену первого лота без TP для расчета TP
            # Для новых единиц (добавленных в шаге 3) лучше использовать last_known_market_price
            # Но если lots_for_new_tp это существующие лоты без TP, используем их entry_price
            # Простое решение: используем entry_price первого лота из lots_for_new_tp
            base_price_for_new_tp = lots_for_new_tp[0]['entry_price']
    
            # Размещаем один TP ордер на все недостающие единицы
            new_tp_order_id = await self._place_individual_take_profit_order(
                client, account_id, base_price_for_new_tp, units_for_new_tp_total
            )
    
            if new_tp_order_id:
                # Привязываем этот TP ордер ко всем лотам без TP (новым)
                for lot in lots_for_new_tp:
                    lot['tp_order_id'] = new_tp_order_id
                    # Обновляем ID в основном списке reconciled_open_lots (он же lot)
                    # (reconciled_open_lots это список словарей, изменения отражаются)
                self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: TP ордер {new_tp_order_id} по упрощенной логике размещен успешно на {units_for_new_tp_total} единиц по цене {base_price_for_new_tp:.{self.display_precision}f}.")
            else:
                self.logger.error(f"[{account_state.account_name_for_log}] SYNC_TP: НЕ УДАЛОСЬ РАЗМЕСТИТЬ TP ордер по упрощенной логике на {units_for_new_tp_total} единиц.")
        elif units_for_new_tp_total == 0:
             self.logger.info(f"[{account_state.account_name_for_log}] SYNC_TP: Нет новых единиц для размещения TP по упрощенной логике.")

        self.logger.debug(f"[{account_state.account_name_for_log}] SYNC_TP: Завершена улучшенная синхронизация TP ордеров.")
                # --- Шаг 5: Обновление флагов состояния бота на основе синхронизированной позиции ---
        # Recalculate internal_total_quantity after potential additions/removals
        ...
        if internal_total_quantity > 0:
            account_state.account_started = True
            account_state.initial_orders_placed = True # If there's a position, we've "started" and "placed initial orders"
            # === ИЗМЕНЕНО: Всегда используем last_known_market_price для last_position_fill_price ===
            # Это обеспечивает согласованность с логикой размещения ордеров входа и TP.
            # Не используем current_avg_price_from_api, так как она может быть неактуальной или неточной.
            account_state.last_position_fill_price = last_known_market_price # <-- Исправлено!
            # ========================================================================
    ...
    # ==========================================================================
    # === КОНЕЦ НОВЫХ МЕТОДОВ ДЛЯ РЕФАКТОРИНГА ===
    # ==========================================================================
    ...
    async def manage_account_orders(self, client, account_id: str, current_market_price: Decimal):
      ...
      # --- Логика определения необходимости перестановки ордеров для входа ---
        ...
        effective_base_price_for_placement = current_market_price # Use the single market price from TradingBot
         ...
         # Проверяем активность ордеров на бирже через ЗАПРОС К API, а не внутреннее состояние
        upper_id, upper_is_stop = account_state.active_entry_order_above
        lower_id, lower_is_stop = account_state.active_entry_order_below
 
        has_upper_entry_order_active = False
        if upper_id:
             # --- ИЗМЕНЕНО: Проверяем статус НАПРЯМУЮ через API ---
             try:
                 if upper_is_stop:
                     # Для стоп-ордеров проверяем в списке стоп-ордеров
                     stop_orders_response = await client.stop_orders.get_stop_orders(
                         account_id=account_id, 
                         status=StopOrderStatusOption.STOP_ORDER_STATUS_ACTIVE
                     )
                     # Ищем наш ордер среди активных
                     if any(so.stop_order_id == upper_id and so.figi == self.figi for so in stop_orders_response.stop_orders):
                         has_upper_entry_order_active = True
                         self.logger.debug(f"[{account_state.account_name_for_log}] Верхний ордер для входа (ID: {upper_id}, STOP) активен на бирже.")
                     else:
                         # Проверим, может он уже исполнен или отменен
                         executed_orders_response = await client.stop_orders.get_stop_orders(
                             account_id=account_id, 
                             status=StopOrderStatusOption.STOP_ORDER_STATUS_EXECUTED
                         )
                         cancelled_orders_response = await client.stop_orders.get_stop_orders(
                             account_id=account_id, 
                             status=StopOrderStatusOption.STOP_ORDER_STATUS_CANCELED
                         )
                         all_inactive = (
                             any(so.stop_order_id == upper_id for so in executed_orders_response.stop_orders) or
                             any(so.stop_order_id == upper_id for so in cancelled_orders_response.stop_orders)
                         )
                         if all_inactive:
                             self.logger.info(f"Для {account_state.account_name_for_log}: Верхний ордер для входа (ID: {upper_id}, STOP) неактивен на бирже (исполнен/отменен).")
                         else:
                             self.logger.info(f"Для {account_state.account_name_for_log}: Верхний ордер для входа (ID: {upper_id}, STOP) не найден на бирже (возможно, удален вручную).")
                         account_state.active_entry_order_above = (None, None) # Очищаем внутренний ID
                 else:
                     # Для обычных лимитных ордеров
                     orders_response = await client.orders.get_orders(account_id=account_id)
                     # Ищем наш ордер среди активных (предполагаем, что ACTIVE и PARTIALLY_FILLED - активные)
                     if any(o.order_id == upper_id and o.figi == self.figi and 
                            o.execution_report_status in [OrderExecutionReportStatus.EXECUTION_REPORT_STATUS_NEW, 
                                                          OrderExecutionReportStatus.EXECUTION_REPORT_STATUS_PARTIALLYFILL] 
                            for o in orders_response.orders):
                         has_upper_entry_order_active = True
                         self.logger.debug(f"[{account_state.account_name_for_log}] Верхний ордер для входа (ID: {upper_id}, LIMIT) активен на бирже.")
                     else:
                         # Проверим, может он уже исполнен или отменен
                         if any(o.order_id == upper_id and o.figi == self.figi and 
                                o.execution_report_status in [OrderExecutionReportStatus.EXECUTION_REPORT_STATUS_FILL, 
                                                              OrderExecutionReportStatus.EXECUTION_REPORT_STATUS_CANCELLED] 
                                for o in orders_response.orders):
                             self.logger.info(f"Для {account_state.account_name_for_log}: Верхний ордер для входа (ID: {upper_id}, LIMIT) неактивен на бирже (исполнен/отменен).")
                         else:
                              self.logger.info(f"Для {account_state.account_name_for_log}: Верхний ордер для входа (ID: {upper_id}, LIMIT) не найден на бирже (возможно, удален вручную).")
                         account_state.active_entry_order_above = (None, None) # Очищаем внутренний ID
             except RequestError as e:
                 self.logger.warning(f"[{account_state.account_name_for_log}] Ошибка API при проверке верхнего ордера {upper_id}: {e}")
                 # В случае ошибки API лучше предположить, что ордер не активен, чтобы не блокировать перестановку
                 account_state.active_entry_order_above = (None, None)
             # --- Конец изменения ---
        ...
        if lower_id:
             ...
             try:
                 if lower_is_stop:
                     ...
                     if any(so.stop_order_id == lower_id and so.figi == self.figi for so in stop_orders_response.stop_orders):
                         has_lower_entry_order_active = True
                         self.logger.debug(f"[{account_state.account_name_for_log}] Нижний ордер для входа (ID: {lower_id}, STOP) активен на бирже.")
                     else:
                         executed_orders_response = await client.stop_orders.get_stop_orders(
                             account_id=account_id, 
                             status=StopOrderStatusOption.STOP_ORDER_STATUS_EXECUTED
                         ...
                         all_inactive = (
                             any(so.stop_order_id == lower_id for so in executed_orders_response.stop_orders) or
                             any(so.stop_order_id == lower_id for so in cancelled_orders_response.stop_orders)
                         )
                         if all_inactive:
                             self.logger.info(f"Для {account_state.account_name_for_log}: Нижний ордер для входа (ID: {lower_id}, STOP) неактивен на бирже (исполнен/отменен).")
                         else:
                             self.logger.info(f"Для {account_state.account_name_for_log}: Нижний ордер для входа (ID: {lower_id}, STOP) не найден на бирже (возможно, удален вручную).")
                         account_state.active_entry_order_below = (None, None)
                 else:
                     orders_response = await client.orders.get_orders(account_id=account_id)
                     if any(o.order_id == lower_id and o.figi == self.figi and 
                            o.execution_report_status in [OrderExecutionReportStatus.EXECUTION_REPORT_STATUS_NEW, 
                                                          OrderExecutionReportStatus.EXECUTION_REPORT_STATUS_PARTIALLYFILL] 
                            for o in orders_response.orders):
                         has_lower_entry_order_active = True
                         self.logger.debug(f"[{account_state.account_name_for_log}] Нижний ордер для входа (ID: {lower_id}, LIMIT) активен на бирже.")
                     else:
                         if any(o.order_id == lower_id and o.figi == self.figi and 
                                o.execution_report_status in [OrderExecutionReportStatus.EXECUTION_REPORT_STATUS_FILL, 
                                                              OrderExecutionReportStatus.EXECUTION_REPORT_STATUS_CANCELLED] 
                                for o in orders_response.orders):
                             self.logger.info(f"Для {account_state.account_name_for_log}: Нижний ордер для входа (ID: {lower_id}, LIMIT) неактивен на бирже (исполнен/отменен).")
                         else:
                              self.logger.info(f"Для {account_state.account_name_for_log}: Нижний ордер для входа (ID: {lower_id}, LIMIT) не найден на бирже (возможно, удален вручную).")
                         account_state.active_entry_order_below = (None, None)
             except RequestError as e:
                 self.logger.warning(f"[{account_state.account_name_for_log}] Ошибка API при проверке нижнего ордера {lower_id}: {e}")
                 account_state.active_entry_order_below = (None, None)
             # --- Конец изменения ---
        ...
        if account_state.orders_need_reposition:
             self.logger.debug(f"[{account_state.account_name_for_log}] Флаг 'orders_need_reposition' установлен. Инициируем перестановку ордеров.")
             ...
             if not has_upper_entry_order_active or not has_lower_entry_order_active:
                 self.logger.debug(f"[{account_state.account_name_for_log}] Прошло достаточно времени ({int((datetime.now() - account_state.last_orders_placement_time).total_seconds())}s) и один из ордеров сетки неактивен. Инициируем перестановку.")
                 should_reposition_orders = True
             else:
...
# =================================================================================
    ...
    async def cancel_all_orders_for_account(self, client: AsyncClient, account_id: str, account_name_for_log: str, 
        ...
        """
        self.logger.debug(f"Отмена активных заявок для счета {account_name_for_log} ({account_id})...")
    ...
    def __init__(self):
        ...
        if ACCOUNT_FOR_TRADING == 2 or ACCOUNT_FOR_TRADING == 3:
            self.sell_account_manager = AccountManager(
                ...
                max_free_margin_rub=MAX_FREE_MARGIN_RUB_FOR_SELL,
                end_of_session_reduction_time=END_OF_SESSION_REDUCTION_TIME,
                check_grid_interval_seconds=CHECK_GRID_INTERVAL_SECONDS,
                logger=self.logger,
        ...
        if ACCOUNT_FOR_TRADING == 1 or ACCOUNT_FOR_TRADING == 3:
            self.buy_account_manager = AccountManager(
                ...
                max_free_margin_rub=MAX_FREE_MARGIN_RUB_FOR_BUY,
                end_of_session_reduction_time=END_OF_SESSION_REDUCTION_TIME,
                check_grid_interval_seconds=CHECK_GRID_INTERVAL_SECONDS,
                logger=self.logger,
                order_manager=self.order_manager # Передача ссылки на OrderManager
        ...
        while self.running: 
            try:
                ...
                if not self.orders_stream_connected: # If already disconnected, log at DEBUG
                    ...
                    self.logger.warning(f"!!!!!!Связь потеряна!!!!!!!!!!! Ошибка в стриме ордеров. Попытка переподключения через {self.orders_reconnect_delay} секунд.")
        ...
        while self.running: 
            try:
                ...
                if not self.market_data_stream_connected: # If already disconnected, log at DEBUG
                    ...
                    self.logger.warning(f"!!!!!!Связь потеряна!!!!!!!!!!! Ошибка в потоке рыночных данных. Попытка переподключения через {self.market_data_reconnect_delay} секунд.")
    ...
    async def run(self):
        async with AsyncClient(API_TOKEN, target=INVEST_GRPC_API) as client:
            try:
                ...
                # --- Изменено на ПАРАЛЛЕЛЬНУЮ синхронизацию ---
                ...
                if self.sell_account_manager:
                    # Для SELL создаем асинхронную функцию, которая выполнит всю работу по синхронизации
                    async def sync_sell():
                        ...
                        self.logger.debug(f"[SELL_ACCOUNT] Начальная синхронизация завершена.")
                    sync_tasks.append(sync_sell())
                ...
                if self.buy_account_manager:
                    ...
                    async def sync_buy():
                        ...
                        self.logger.debug(f"[BUY_ACCOUNT] Начальная синхронизация завершена.")
                    sync_tasks.append(sync_buy())
...
async def run_bot():
    ...
    print(f"Максимальный дневной убыток для SELL счета: {MAX_DAILY_LOSS_RUB_SELL} RUB")
    print(f"Максимальный дневной убыток для BUY счета: {MAX_DAILY_LOSS_RUB_BUY} RUB")
    print(f"Максимальная маржа для BUY: {MAX_FREE_MARGIN_RUB_FOR_BUY} RUB")
    print(f"Максимальная маржа для SELL: {MAX_FREE_MARGIN_RUB_FOR_SELL} RUB")
